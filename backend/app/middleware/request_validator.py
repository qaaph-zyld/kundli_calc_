...
import pytz
from app.core.errors.error_messages import ErrorCode, ErrorMessages
...
def validate_ayanamsa_system(system: str) -> bool:
    ...
    valid_systems = ["lahiri", "raman", "kp", "yukteshwar", "true_chitra", "fagan_bradley", "deluce", "sassanian"]
    ...
    def validate_chart_request(data: dict) -> dict:
            ...
            if isinstance(date_value, str):
                if "T" in date_value or "Z" in date_value:
                    ...
                    if not validate_date_format(date_value):
                        errors.append(
                            ErrorMessages.format_validation_error(date_field, date_value, ErrorCode.INVALID_DATE_FORMAT)
                        )
                    elif not validate_birth_date_not_future(date_value):
                        errors.append(
                            ErrorMessages.format_validation_error(date_field, date_value, ErrorCode.FUTURE_DATE)
                        )
        ...
        if "time" in data:
            ...
            if not validate_time_format(time_value):
                errors.append(ErrorMessages.format_validation_error("time", time_value, ErrorCode.INVALID_TIME_FORMAT))
        ...
        if "latitude" in data and "longitude" in data:
            ...
            if not validate_coordinates(lat, lon):
                errors.append(
                    ErrorMessages.format_validation_error(
                        "coordinates", f"lat={lat}, lon={lon}", ErrorCode.INVALID_COORDINATES
                    )
                )
        ...
        if "timezone" in data:
            ...
            if not validate_timezone(tz):
                errors.append(ErrorMessages.format_validation_error("timezone", tz, ErrorCode.INVALID_TIMEZONE))

        # Validate ayanamsa
        if "ayanamsa" in data:
            ayanamsa = data["ayanamsa"]
            if isinstance(ayanamsa, str) and not validate_ayanamsa_system(ayanamsa):
                errors.append(ErrorMessages.format_validation_error("ayanamsa", ayanamsa, ErrorCode.INVALID_AYANAMSA))
        ...
        if "house_system" in data:
            ...
            if not validate_house_system(house_sys):
                errors.append(
                    ErrorMessages.format_validation_error("house_system", house_sys, ErrorCode.INVALID_HOUSE_SYSTEM)
                )
...
async def validation_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content=exc.detail if isinstance(exc.detail, dict) else {"error": True, "message": str(exc.detail)},
    )

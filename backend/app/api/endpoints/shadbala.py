...
class ShadbalaRequest(BaseModel):
    ...
    aspects: List[AspectData] = Field(..., description="List of aspects to the planet")
    planet_positions: Dict[str, float] = Field(..., description="Positions of all planets")
...
class ShadbalaAnalysisRequest(BaseModel):
    ...
    birth_time_is_day: bool = Field(..., description="Whether birth time is during day")
    planet_positions: Dict[str, float] = Field(..., description="Positions of all planets")
    aspects: Dict[str, List[AspectData]] = Field(..., description="Aspects for each planet")
    house_positions: Dict[str, int] = Field(..., description="House positions of planets")


class CompleteShadbalaRequest(BaseModel):
    """Request model for complete BPHS-compliant Shadbala calculation"""

    planet_positions: Dict[str, float] = Field(..., description="Tropical planetary longitudes in degrees")
    house_cusps: List[float] = Field(..., description="List of 12 house cusps in tropical degrees")
    birth_datetime: str = Field(..., description="Birth datetime in ISO format (YYYY-MM-DDTHH:MM:SS)")
    latitude: float = Field(..., description="Birth latitude")
    longitude: float = Field(..., description="Birth longitude")
    ayanamsa: Optional[float] = Field(default=23.85, description="Ayanamsa value (default Lahiri ~23.85°)")
...
async def calculate_shadbala(request: ShadbalaRequest):
    ...
    try:
        ...
        result = shadbala_calculator.calculate_shadbala(request.planet, request.house, speed, aspects, request.is_day)
...
async def calculate_complete_shadbala(request: CompleteShadbalaRequest):
    ...
    try:
        ...
        total_rupas = sum(r.get("total_rupas", 0) for r in results.values())
        strong_planets = [planet for planet, data in results.items() if data.get("is_strong", False)]
        weak_planets = [planet for planet, data in results.items() if not data.get("is_strong", False)]
...
async def analyze_all_planets(request: ShadbalaAnalysisRequest):
    ...
    try:
        ...
        for planet, position in request.planet_positions.items():
            ...
            result = shadbala_calculator.calculate_shadbala(planet, house, speed, aspects, request.birth_time_is_day)
            result["interpretation"] = _interpret_shadbala(result)

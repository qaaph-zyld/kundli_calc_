...
from app.core.calculations.kp_system import KPPosition, KPSystem, get_kp_data
...
async def calculate_kp_data(request: KPRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        result = get_kp_data(planets=request.planets, house_cusps=request.house_cusps, current_time=birth_time)

        return {"success": True, "data": result, "system": "Krishnamurti Paddhati (KP)"}
...
async def get_ruling_planets(request: RulingPlanetsRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        rp = kp.get_ruling_planets(current_time, request.moon_longitude, request.ascendant_longitude)
...
async def get_kp_ayanamsa(
    year: int = Query(..., description="Year"), month: int = Query(1, ge=1, le=12), day: int = Query(1, ge=1, le=31)
) -> Dict[str, float]:
    """

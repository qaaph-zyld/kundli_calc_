...
def create_birth_chart(chart: BirthChartCreate, db: Session = Depends(get_db)):
...
def read_birth_chart(chart_id: UUID, db: Session = Depends(get_db)):
...
def read_user_birth_charts(
    user_id: UUID, skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=100), db: Session = Depends(get_db)
):
    """Get all birth charts for a specific user."""
    return birth_charts.get_user_birth_charts(db, user_id=user_id, skip=skip, limit=limit)


@router.put("/{chart_id}", response_model=BirthChart)
def update_birth_chart(chart_id: UUID, chart: BirthChartUpdate, db: Session = Depends(get_db)):
...
def delete_birth_chart(chart_id: UUID, db: Session = Depends(get_db)):

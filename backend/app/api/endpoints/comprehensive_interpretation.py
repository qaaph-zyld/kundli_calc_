    ...
    try:
        ...
        for placement in request.placements:
            ...
            try:
                ...
                planet_data["interpretation"] = {"note": "No BPHS interpretation available for this placement"}

            # Add multi-source comparison if requested and available
            if request.include_multi_source:
                sources_available = multi_source_engine.get_available_sources(placement.planet, placement.house)

                if len(sources_available) > 1:
                    comparison = multi_source_engine.compare_sources(placement.planet, placement.house)
        ...
        if len(request.placements) > 0:
            result["chart_analysis"]["average_confidence"] = round(total_confidence / len(request.placements), 2)
        ...
        result["synthesis"] = generate_chart_synthesis(
            request.placements, result["chart_analysis"]["average_confidence"], multi_source_count
        )
        ...
        raise HTTPException(status_code=500, detail=f"Error generating comprehensive interpretation: {str(e)}")
...
async def demo_comprehensive_interpretation():
    ...
    result["demo_note"] = (
        "This demonstrates comprehensive chart interpretation combining all available knowledge sources with full source attribution"
    )
...
def generate_chart_synthesis(placements: List[ChartPlacement], avg_confidence: float, multi_source_count: int) -> str:
    """Generate synthesized interpretation for chart"""
    synthesis_parts = []

    synthesis_parts.append(
        f"Chart analysis based on {len(placements)} planetary placements " f"from classical Vedic astrology texts."
    )

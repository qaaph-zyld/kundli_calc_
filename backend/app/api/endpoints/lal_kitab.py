    ...
    try:
        ...
        for planet in request.planet_houses.keys():
            if planet not in PLANETS:
                raise HTTPException(status_code=400, detail=f"Invalid planet name: {planet}. Valid planets: {PLANETS}")

        # Validate house numbers
        for planet, house in request.planet_houses.items():
            if not 1 <= house <= 12:
                raise HTTPException(
                    status_code=400, detail=f"Invalid house number for {planet}: {house}. Must be 1-12."
                )

        analysis = get_lal_kitab_analysis(request.planet_houses)

        return {"success": True, "system": "Lal Kitab", "data": analysis}
...
async def get_specific_remedy(request: PlanetRemedyRequest) -> Dict[str, Any]:
    ...
    try:
        if request.planet not in PLANETS:
            raise HTTPException(status_code=400, detail=f"Invalid planet: {request.planet}. Valid: {PLANETS}")

        remedy = get_planet_remedy(request.planet, request.house)

        return {"success": True, "data": remedy}
...
async def get_planet_info(planet: str) -> Dict[str, Any]:
    ...
    """
    if planet not in PLANETS:
        raise HTTPException(status_code=400, detail=f"Invalid planet: {planet}. Valid: {PLANETS}")

    from app.core.calculations.lal_kitab import DEBILITATED_HOUSES, ENEMIES, EXALTED_HOUSES, FRIENDS
    ...
    return {
        ...
        "remedies": {"hindi": remedies.get("hindi", []), "english": remedies.get("english", [])},
    }
        ...
        debts_info.append(
            {
                ...
                "remedies": {"hindi": info["remedies_hindi"], "english": info["remedies_english"]},
            }
        )

    return {"success": True, "karmic_debts": debts_info}
    ...
    for planet, remedy_data in LAL_KITAB_REMEDIES.items():
        ...
        if language == "hindi":
            all_remedies[planet] = {
                ...
                "house_specific": {h: r.get("hindi", "") for h, r in afflicted.items()},
            }
        elif language == "english":
            all_remedies[planet] = {
                "general": general.get("english", []),
                "house_specific": {h: r.get("english", "") for h, r in afflicted.items()},
            }
        else:
            all_remedies[planet] = {
                "general": {"hindi": general.get("hindi", []), "english": general.get("english", [])},
                "house_specific": {
                    h: {"hindi": r.get("hindi", ""), "english": r.get("english", "")} for h, r in afflicted.items()
                },
            }

    return {"success": True, "language": language, "remedies": all_remedies}

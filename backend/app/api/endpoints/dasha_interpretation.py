...
class DashaInterpretationRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"planet": "Sun", "house": 10, "sign": "Aries", "dignity": "exalted", "current_dasha": "Sun"}
        }
    )
...
async def get_dasha_integrated_interpretation(request: DashaInterpretationRequest):
    ...
    try:
        ...
        response = {
            ...
            "timing_analysis": {
                ...
                "activation_status": (
                    ...
                    if is_active_now
                    else f"Future activation - These effects will manifest during {request.planet} mahadasha"
                ),
            ...
            "integrated_guidance": generate_integrated_guidance(
                request.planet, request.house, request.current_dasha, is_active_now, natal_result, dasha_data
            ),
            "sources": {"natal": natal_result.sources_used, "dasha": ["Jataka Parijata Ch. 8-9"]},
            "status": "success",
        }
        ...
        raise HTTPException(status_code=500, detail=f"Error generating dasha-integrated interpretation: {str(e)}")
...
async def demo_dasha_integrated():
    ...
    demo_request = DashaInterpretationRequest(
        planet="Sun", house=10, sign="Aries", dignity="exalted", current_dasha="Sun"
    )
...
async def get_current_dasha_effects(planet: str = Path(..., description="Mahadasha planet name")):
    """Get detailed dasha period effects"""
    try:
        dasha_data = get_dasha_interpretation(planet)

        if not dasha_data:
            raise HTTPException(status_code=404, detail=f"No dasha interpretation available for {planet}")
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving dasha effects: {str(e)}")


def generate_integrated_guidance(
    planet: str, house: int, current_dasha: str, is_active: bool, natal_result, dasha_data: dict
) -> dict:
    """Generate integrated guidance combining natal + dasha"""

    guidance = {"timing_window": "", "focus_areas": [], "recommendations": []}

...
from app.core.calculations.transit_analysis import TransitAnalyzer, analyze_transits, get_current_transits_summary
...
async def get_major_transits(request: MajorTransitsRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "natal_moon_sign": request.natal_moon_sign, "major_transits": summary}
...
async def get_gochara_rules() -> Dict[str, Any]:
    ...
    return {
        ...
        "rules": {
            ...
            "Rahu": {"benefic_houses": [3, 6, 10, 11], "vedha_points": {}, "note": "Material gains in upachaya houses"},
            "Ketu": {
                "benefic_houses": [3, 6, 10, 11],
                "vedha_points": {},
...
async def get_transit_predictions(request: TransitRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        life_areas = {"career": [], "relationships": [], "finances": [], "health": [], "spirituality": []}
        ...
        for gr in result["gochara_results"]:
            ...
            if house in [9, 12] or planet == "Ketu":
                if is_benefic:
                    ...
                    life_areas["spirituality"].append(
                        f"{planet} transit may bring spiritual insights through challenges"
                    )
...
def _get_general_transit_advice(score: float) -> str:
    ...
    if score >= 75:
        ...
        return (
            "Challenging transit period. Focus on consolidation rather than expansion. Remedial measures recommended."
        )
    else:
...
async def calculate_transit_aspects(request: TransitAspectsRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        aspects = get_transit_to_natal_aspects(transit_pos, request.natal_positions, request.orb)
...
async def get_upcoming_transit_events(request: UpcomingTransitsRequest) -> Dict[str, Any]:
    ...
    """
    try:
        upcoming = get_upcoming_transits(request.natal_moon_sign, request.days_ahead)

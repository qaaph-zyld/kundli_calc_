...
class BhavaAnalysisRequest(BaseModel):
    ...
    planet_positions: Dict[str, float] = Field(..., description="Dictionary of planet positions in degrees (0-360)")
    aspects: Dict[str, List[int]] = Field(..., description="Dictionary of planet aspects to houses")
...
async def analyze_bhava_chart(request: BhavaAnalysisRequest) -> Dict[str, Any]:
    ...
    """
    try:
        result = BhavaSystem.analyze_bhava_chart(request.planet_positions, request.aspects)
        return result
...
async def analyze_house(house_number: int, request: BhavaAnalysisRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result = BhavaSystem.calculate_house_strength(house_number, request.planet_positions, request.aspects)

        # Add house relationships
        result["relationships"] = BhavaSystem.get_house_relationships(house_number)

        return {"house_number": house_number, "analysis": result}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

...
class AshtakavargaRequest(BaseModel):
    ...
    planet_positions: Dict[str, int] = Field(..., description="Dictionary of planet positions in houses (1-12)")
...
class CompleteAshtakavargaRequest(BaseModel):
    ...
    planet_longitudes: Dict[str, float] = Field(..., description="Sidereal planetary longitudes in degrees (0-360)")
    ascendant: float = Field(..., description="Ascendant/Lagna longitude in sidereal degrees")
...
async def calculate_ashtakavarga(request: AshtakavargaRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        sarvashtakavarga = Ashtakavarga.calculate_sarvashtakavarga(request.planet_positions)

        # Analyze each planet's strength
        planet_analysis = {}
        for planet in request.planet_positions.keys():
            analysis = Ashtakavarga.analyze_planet_strength(planet, sarvashtakavarga)
...
async def analyze_planet(planet: str, request: AshtakavargaRequest) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"planet": planet, "analysis": analysis, "bindus_per_house": sarvashtakavarga.get(planet, [])}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def calculate_complete_ashtakavarga_endpoint(request: CompleteAshtakavargaRequest) -> Dict[str, Any]:
    ...
    try:
        result = calculate_complete_ashtakavarga(
            planet_longitudes=request.planet_longitudes, ascendant=request.ascendant
        )

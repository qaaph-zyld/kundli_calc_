...
async def calculate_horoscope(request: HoroscopeRequest):
    ...
    try:
        ...
        location = Location(latitude=request.latitude, longitude=request.longitude, altitude=request.altitude)
        ...
        planetary_positions = astro_calc.calculate_planetary_positions(date=birth_time, location=location)

        # Calculate ayanamsa
        ayanamsa_value = ayanamsa_calc.calculate_precise_ayanamsa(
            datetime_utc=birth_time, system=request.ayanamsa_system
        )
        ...
        if request.divisional_charts:
            divisional_charts = divisional_calc.calculate_all_divisions(
                planetary_positions=planetary_positions, ayanamsa_value=ayanamsa_value, charts=request.divisional_charts
            )

        # Calculate planetary strengths
        planetary_strengths = strength_calc.calculate_complete_strengths(
            planetary_positions=planetary_positions, birth_time=birth_time, location=location
        )

        # Calculate aspects
        aspects = aspect_calc.calculate_aspects(planetary_positions=planetary_positions, ayanamsa_value=ayanamsa_value)

        # Analyze houses
        house_analysis = {}
        for house in range(1, 13):  # 12 houses
            house_analysis[str(house)] = house_calc.analyze_house(
                house_number=house, planetary_positions=planetary_positions, ayanamsa_value=ayanamsa_value
            )
...
async def batch_calculations(
    start_date: datetime, end_date: datetime, location: Location, calculation_type: str = "planetary_positions"
) -> List[Dict[str, Any]]:
    """
    ...
    """
    try:
        results = batch_processor.process_date_range(start_date, end_date, location, calculation_type)
        return results
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Error in batch calculations: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error during batch calculations")

...
async def get_planet_in_house_interpretation(
    ...
    sources: Optional[str] = Query(
        None, description="Comma-separated source names to include (BPHS,Saravali,Phaladeepika,Hora_Sara)"
    ),
):
    """
    ...
    if planet not in valid_planets:
        raise HTTPException(status_code=400, detail=f"Invalid planet. Must be one of: {', '.join(valid_planets)}")

    if house < 1 or house > 12:
        raise HTTPException(status_code=400, detail="Invalid house. Must be between 1 and 12")
    ...
    if sources:
        ...
        result["sources"] = {k: v for k, v in result["sources"].items() if k in requested_sources}
        result["source_count"] = len(result["sources"])

    if result["source_count"] == 0:
        raise HTTPException(status_code=404, detail=f"No interpretations found for {planet} in house {house}")
...
async def get_knowledge_base_statistics():
    ...
    total_interpretations = sum(source_data["count"] for source_data in stats["coverage_by_source"].values())
...
async def search_interpretations(
    ...
    return {
        "keyword": keyword,
        "filters": {"planet": planet, "house": house, "source": source},
...
async def compare_planet_across_houses(planet: str):
    ...
    result = {
        "planet": planet,
        "houses": {},
        "summary": {"total_houses": 12, "houses_with_data": 0, "best_houses": [], "challenging_houses": []},
    }

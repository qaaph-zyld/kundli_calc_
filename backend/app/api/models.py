...
class Location(BaseModel):
    ...
    latitude: Decimal = Field(..., description="Latitude of the location", ge=-90, le=90, examples=[13.0827])  # Chennai
    longitude: Decimal = Field(
        ..., description="Longitude of the location", ge=-180, le=180, examples=[80.2707]  # Chennai
    )
    altitude: Decimal = Field(default=Decimal("0"), description="Altitude in meters", examples=[0])


class ChartRequest(BaseModel):
    """Request model for birth chart calculation."""

    date_time: datetime = Field(..., description="Date and time in UTC", examples=["2024-01-01T12:00:00Z"])
    latitude: Decimal = Field(..., description="Latitude of birth location", ge=-90, le=90)
    longitude: Decimal = Field(..., description="Longitude of birth location", ge=-180, le=180)
    altitude: Decimal = Field(default=Decimal("0"), description="Altitude in meters")
    ayanamsa: int = Field(default=1, description="Ayanamsa system (1: Lahiri, 2: Raman, 3: KP)")
    ...
    house_system: str = Field(default="W", description="House system (P: Placidus, K: Koch, R: Regiomontanus)")

    @field_validator("date_time")
    @classmethod
    def normalize_date_time_to_utc_naive(cls, v: datetime) -> datetime:
        """Normalize timezone-aware datetimes to UTC naive for Swiss Ephemeris."""
        if v.tzinfo is None:
            return v
...
class ChartResponse(BaseModel):
    ...
    planetary_positions: Dict[str, PlanetaryPosition] = Field(..., description="Positions of all planets")
    houses: House = Field(..., description="House system data")
    aspects: List[AspectInfluence] = Field(default_factory=list, description="Detailed planetary aspects")
    ayanamsa_value: Decimal = Field(..., description="Calculated ayanamsa value")
    planetary_strengths: Dict[str, PlanetaryStrength] = Field(
        ..., description="Detailed strength calculations for each planet"
    )
    divisional_charts: Dict[str, DivisionalChart] = Field(..., description="Calculated divisional charts")

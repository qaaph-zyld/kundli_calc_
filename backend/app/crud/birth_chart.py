...
def get_birth_charts_by_user(db: Session, user_id: str, skip: int = 0, limit: int = 100) -> List[BirthChart]:
    """Get birth charts by user ID."""
    return db.query(BirthChart).filter(BirthChart.user_id == user_id).offset(skip).limit(limit).all()


def create_birth_chart(db: Session, birth_chart: BirthChartCreate, user_id: str) -> BirthChart:
    """Create new birth chart."""
    db_birth_chart = BirthChart(id=str(uuid.uuid4()), user_id=user_id, **birth_chart.model_dump())
...
def update_birth_chart(db: Session, birth_chart_id: str, birth_chart: BirthChartUpdate) -> Optional[BirthChart]:
    """Update birth chart."""

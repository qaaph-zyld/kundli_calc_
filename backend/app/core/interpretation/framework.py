...
class AstrologicalInterpreter:
    ...
    def interpret_chart(
        self, positions: Dict[CelestialBody, PlanetaryPosition], birth_time: datetime, location: GeoLocation
        ...
        dasha_periods = self.algorithms.calculate_dasha_periods(positions, birth_time)
        ...
        for domain in InterpretationDomain:
            domains[domain] = self._interpret_domain(domain, positions, all_yogas, dasha_periods, strengths)

        # Calculate overall strength
        overall_strength = self._calculate_overall_strength(domains, all_yogas, strengths)
    ...
    def _interpret_domain(
        ...
        relevant_yogas = [
            yoga for yoga in yogas if any(p in planets for p in yoga.planets) or any(h in houses for h in yoga.houses)
        ]

        # Filter relevant dashas
        relevant_dashas = [dasha for dasha in dashas if dasha.major_planet in planets]

        # Calculate domain strength
        strength = self._calculate_domain_strength(domain, positions, relevant_yogas, strengths)

        # Generate interpretation
        description = self._generate_domain_description(domain, positions, relevant_yogas, strength)

        # Generate recommendations
        recommendations = self._generate_domain_recommendations(domain, strength, relevant_yogas, relevant_dashas)

        # Identify supporting factors
        factors = self._identify_supporting_factors(domain, positions, relevant_yogas, strengths)
    ...
    def _get_domain_houses(self, domain: InterpretationDomain) -> Set[House]:
        """Get houses relevant to domain"""
        domain_houses = {
            InterpretationDomain.PERSONALITY: {House.FIRST, House.FOURTH, House.TENTH},
            InterpretationDomain.CAREER: {House.TENTH, House.SIXTH, House.SECOND},
            InterpretationDomain.RELATIONSHIPS: {House.SEVENTH, House.FIFTH, House.ELEVENTH},
            InterpretationDomain.HEALTH: {House.FIRST, House.SIXTH, House.EIGHTH},
            InterpretationDomain.SPIRITUALITY: {House.NINTH, House.TWELFTH, House.FOURTH},
            InterpretationDomain.WEALTH: {House.SECOND, House.ELEVENTH, House.FIFTH},
            InterpretationDomain.EDUCATION: {House.FOURTH, House.FIFTH, House.NINTH},
            InterpretationDomain.FAMILY: {House.FOURTH, House.SECOND, House.THIRD},
        }
        return domain_houses[domain]

    def _get_domain_planets(self, domain: InterpretationDomain) -> Set[CelestialBody]:
        """Get planets relevant to domain"""
        domain_planets = {
            InterpretationDomain.PERSONALITY: {CelestialBody.SUN, CelestialBody.MOON, CelestialBody.MARS},
            InterpretationDomain.CAREER: {CelestialBody.SATURN, CelestialBody.SUN, CelestialBody.JUPITER},
            InterpretationDomain.RELATIONSHIPS: {CelestialBody.VENUS, CelestialBody.JUPITER, CelestialBody.MOON},
            InterpretationDomain.HEALTH: {CelestialBody.SUN, CelestialBody.MARS, CelestialBody.SATURN},
            InterpretationDomain.SPIRITUALITY: {CelestialBody.JUPITER, CelestialBody.KETU, CelestialBody.SATURN},
            InterpretationDomain.WEALTH: {CelestialBody.JUPITER, CelestialBody.VENUS, CelestialBody.MERCURY},
            InterpretationDomain.EDUCATION: {CelestialBody.MERCURY, CelestialBody.JUPITER, CelestialBody.VENUS},
            InterpretationDomain.FAMILY: {CelestialBody.MOON, CelestialBody.VENUS, CelestialBody.JUPITER},
        }
    ...
    def _calculate_domain_strength(
        ...
        planet_strengths = [strengths[p].percentage for p in planets if p in strengths]
        avg_planet_strength = sum(planet_strengths) / len(planet_strengths) if planet_strengths else 0

        # Calculate yoga strength
        yoga_strength = sum(yoga.strength for yoga in yogas) / len(yogas) if yogas else 0

        # Calculate house strength
        house_strength = sum(self._calculate_house_strength(h, positions) for h in houses) / len(houses)

        # Weighted average
        total_strength = avg_planet_strength * 0.4 + yoga_strength * 0.4 + house_strength * 0.2
    ...
    def _calculate_house_strength(self, house: House, positions: Dict[CelestialBody, PlanetaryPosition]) -> float:
        """Calculate house strength"""

        strength = 0.0

        # Check planets in house
        planets_in_house = [pos for pos in positions.values() if pos.house == house]
        ...
        for pos in planets_in_house:
            ...
            dignity = self.math.calculate_dignity_score(pos.body, pos.sign, house)
            strength += dignity

            # Points for beneficial aspects
            for other_pos in positions.values():
                if other_pos.body != pos.body:
                    aspect = self.algorithms._check_mutual_aspect(pos, other_pos)
    ...
    def _get_domain_template(self, domain: InterpretationDomain, strength: InterpretationStrength) -> str:
        ...
        return templates.get(domain, {}).get(
            strength, "General interpretation for {domain} with {strength} strength..."
        )
    ...
    def _generate_domain_recommendations(
        ...
        for yoga in yogas[:3]:  # Top 3 yogas
            recommendations.extend(self._get_yoga_recommendations(yoga))

        # Dasha-based recommendations
        for dasha in dashas[:2]:  # Next 2 periods
            recommendations.extend(self._get_dasha_recommendations(dasha))

        return recommendations

    def _get_base_recommendations(self, domain: InterpretationDomain, strength: InterpretationStrength) -> List[str]:
        ...
        return recommendations.get(domain, {}).get(strength, ["General recommendation for domain"])
    ...
    def _identify_supporting_factors(
        ...
        for planet in planets:
            if planet in positions:
                ...
                factors.append(f"{planet.name} in {pos.sign.name} " f"({pos.house.name} House)")

        # Strong yogas
        for yoga in yogas[:3]:
            if yoga.strength >= 60:
                factors.append(f"Strong {yoga.name} " f"({yoga.strength:.1f}% strength)")
        ...
        for planet in planets:
            if planet in strengths:
                ...
                if strength.percentage >= 60:
                    factors.append(f"Strong {planet.name} " f"({strength.percentage:.1f}%)")

        return factors

    def _determine_timeframe(self, dashas: List[DashaResult]) -> InterpretationTimeframe:
        ...
        current_duration = (current_dasha.end_date - current_dasha.start_date).days / 365.25
    ...
    def _calculate_overall_strength(
        ...
        domain_strengths = [domain_values[d.strength] for d in domains.values()]
        avg_domain_strength = sum(domain_strengths) / len(domain_strengths)

        # Yoga strength
        yoga_strength = sum(y.strength for y in yogas) / len(yogas) if yogas else 0

        # Planetary strength
        planet_strength = sum(s.percentage for s in strengths.values()) / len(strengths)

        # Weighted average
        return avg_domain_strength * 0.4 + yoga_strength * 0.4 + planet_strength * 0.2

    def _identify_key_periods(self, dashas: List[DashaResult]) -> List[DashaResult]:
        """Identify key dasha periods"""

        # Sort by strength and get top periods
        sorted_dashas = sorted(dashas, key=lambda x: x.strength, reverse=True)
        return sorted_dashas[:5]  # Top 5 periods

    def _identify_major_yogas(self, yogas: List[YogaResult]) -> List[YogaResult]:
        """Identify major yogas"""

        # Sort by strength and get top yogas
        sorted_yogas = sorted(yogas, key=lambda x: x.strength, reverse=True)

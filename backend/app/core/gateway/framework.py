...
class ServiceRegistry:
    ...
    def get_service_url(self, service_name: str, request: Optional[Request] = None) -> str:
        """Get service URL based on load balance strategy"""
        service = self.config.services[service_name]
        healthy_indices = [i for i, health in enumerate(self.services[service_name]) if health == ServiceHealth.HEALTHY]
        ...
        if strategy == LoadBalanceStrategy.ROUND_ROBIN:
            ...
            index = min(healthy_indices, key=lambda i: self.connections.get(f"{service_name}_{i}", 0))

        elif strategy == LoadBalanceStrategy.WEIGHTED:
            weights = [service.weight if i in healthy_indices else 0 for i in range(len(service.hosts))]
            import random

            index = random.choices(range(len(service.hosts)), weights=weights)[0]
            ...
            if not request:
                ...
                hash_value = int(hashlib.md5(request.client.host.encode()).hexdigest(), 16)
    ...
    async def check_service_health(self, name: str, index: int) -> None:
        ...
        try:
            async with httpx.AsyncClient(verify=service.ssl_verify, timeout=5) as client:
...
class RateLimiter:
    ...
    async def check_rate_limit(self, key: str, limit_type: RateLimitType) -> bool:
...
class APIGateway:
    ...
    def __init__(self, app: FastAPI, config: GatewayConfig, redis_client: redis.Redis):
        self.app = app
        self.config = config
    ...
    def _setup_middleware(self) -> None:
        ...
        # Gateway middleware
        self.app.add_middleware(BaseHTTPMiddleware, dispatch=self._gateway_middleware)

    def _setup_routes(self) -> None:
        """Setup routes"""
        for path, route in self.config.routes.items():
            self.circuit_breakers[path] = CircuitBreaker(threshold=route.circuit_breaker_threshold)
    ...
    async def _check_auth(self, request: Request, route: RouteConfig) -> None:
    ...
    async def _check_rate_limit(self, request: Request, route: RouteConfig) -> None:
        """Check rate limit"""
        ...
        if not await self.rate_limiter.check_rate_limit(key, route.rate_limit_type):
            raise AppError(
    ...
    async def _gateway_middleware(self, request: Request, call_next: Callable) -> Response:
        """Gateway middleware"""
        ...
        try:
            ...
            service_url = self.registry.get_service_url(route_config.upstream_service, request)

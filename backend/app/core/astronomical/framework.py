...
class AstronomicalCalculator:
    ...
    def _setup_ephemeris(self) -> None:
        ...
        # Set ayanamsa
        # Note: SIDM_LAHIRI_1940 matches JHora's Lahiri implementation
        # Default SIDM_LAHIRI has ~0.016° offset from JHora reference
    ...
    def _get_house(self, longitude: float, ascendant: float) -> House:
    ...
    def _calculate_aspect(self, body1_pos: PlanetaryPosition, body2_pos: PlanetaryPosition) -> Optional[AspectPosition]:
        ...
        # Define aspect orbs
        orbs = {Aspect.CONJUNCTION: 10, Aspect.SEXTILE: 6, Aspect.SQUARE: 8, Aspect.TRINE: 8, Aspect.OPPOSITION: 10}
    ...
    def calculate_planet_position(self, body: CelestialBody, dt: datetime, location: GeoLocation) -> PlanetaryPosition:
        ...
        try:
            ...
            topo_flag = getattr(swe, "FLG_TOPOCENTRIC", getattr(swe, "FLG_TOPOCTR", 0))
            # FLG_TRUEPOS gives true geometric position (matches JHora)
            # without aberration correction that FLG_SPEED alone applies
            calc_flags_base = swe.FLG_SWIEPH | swe.FLG_SIDEREAL | swe.FLG_SPEED | swe.FLG_TRUEPOS
            calc_flags = (topo_flag | calc_flags_base) if self.coordinate_system == CoordinateSystem.TOPOCENTRIC else calc_flags_base
            ...
            try:
                ...
                fallback_flags_base = swe.FLG_MOSEPH | swe.FLG_SIDEREAL | swe.FLG_SPEED
                fallback_flags = (
                    (topo_flag | fallback_flags_base)
                    if self.coordinate_system == CoordinateSystem.TOPOCENTRIC
                    else fallback_flags_base
                )
                if body == CelestialBody.KETU:
            ...
            _cusps_tmp, ascmc = swe.houses(jd, location.latitude, location.longitude, b"P")
            ...
            raise AppError(
                ...
                severity=ErrorSeverity.HIGH,
                details={"body": body, "datetime": dt.isoformat(), "location": location.__dict__, "error": str(e)},
            )

    def calculate_all_positions(self, dt: datetime, location: GeoLocation) -> Dict[CelestialBody, PlanetaryPosition]:
        """Calculate positions for all planets"""
        positions = {}
        for body in CelestialBody:
            positions[body] = self.calculate_planet_position(body, dt, location)
        return positions

    def calculate_aspects(self, positions: Dict[CelestialBody, PlanetaryPosition]) -> List[AspectPosition]:
        ...
        for i in range(len(bodies)):
            for j in range(i + 1, len(bodies)):
                ...
                aspect = self._calculate_aspect(positions[body1], positions[body2])
                if aspect:

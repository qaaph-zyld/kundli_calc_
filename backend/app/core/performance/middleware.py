...
class PerformanceMiddleware(BaseMiddleware):
    ...
    async def process_request(self, request: Request) -> Request:
        ...
        await performance_monitor.record_metric(MetricType.THROUGHPUT, 1.0)  # Increment request count
    ...
    async def process_response(self, response: Response) -> Response:
        ...
        if hasattr(response.request.state, "start_time"):
            ...
            # Record latency
            await performance_monitor.record_metric(MetricType.LATENCY, response_time)

            # Record error rate if applicable
            if response.status_code >= 400:
                await performance_monitor.record_metric(MetricType.ERROR_RATE, 1.0)

...
class AstrologicalAlgorithms:
    ...
    def analyze_raja_yoga(self, positions: Dict[CelestialBody, PlanetaryPosition]) -> List[YogaResult]:
        ...
        for trine in trine_houses:
            for quadrant in quadrant_houses:
                ...
                if trine_lord and quadrant_lord:
                    aspect = self._check_mutual_aspect(positions[trine_lord], positions[quadrant_lord])
    ...
    def analyze_dhana_yoga(self, positions: Dict[CelestialBody, PlanetaryPosition]) -> List[YogaResult]:
        """Analyze Dhana Yoga combinations"""
        results = []

        # Check for 2nd and 11th house combinations
        wealth_houses = [House.SECOND, House.ELEVENTH]
        wealth_planets = [CelestialBody.JUPITER, CelestialBody.VENUS, CelestialBody.MERCURY]
            ...
            if house_lord:
                for planet in wealth_planets:
                    if planet in positions:
                        aspect = self._check_mutual_aspect(positions[house_lord], positions[planet])
                        ...
                        if aspect:
                            results.append(
                                YogaResult(
                                    ...
                                    houses=[house],
                                    strength=self._calculate_yoga_strength([positions[house_lord], positions[planet]]),
                                    description="Wealth-generating combination " "between house lord and benefic",
                                    effects=[
    ...
    def analyze_mahapurusha_yoga(self, positions: Dict[CelestialBody, PlanetaryPosition]) -> List[YogaResult]:
        ...
        for planet in mahapurusha_planets:
            if planet in positions:
                ...
                if pos.house in angles:
                    if self._is_in_own_sign(planet, pos.sign) or self._is_in_exaltation(planet, pos.sign):
                        results.append(
                            YogaResult(
                                ...
                                description=f"Great person yoga formed by {planet.name} " f"in angle",
    ...
    def calculate_dasha_periods(
        ...
        if system == DashaSystem.VIMSHOTTARI:
            ...
            for planet, years in sequence:
                ...
                end_date = current_date.fromtimestamp(current_date.timestamp() + duration)
                ...
                results.append(
                    DashaResult(
                        ...
                        end_date=end_date,
                        strength=self._calculate_dasha_strength(positions[planet], positions),
    ...
    def calculate_planetary_strength(
        self, positions: Dict[CelestialBody, PlanetaryPosition], factor: StrengthFactor = StrengthFactor.SHADBALA
    ...
    def _get_house_lord(
        self, positions: Dict[CelestialBody, PlanetaryPosition], house: House
    ...
    def _check_mutual_aspect(self, pos1: PlanetaryPosition, pos2: PlanetaryPosition) -> Optional[Aspect]:
    ...
    def _calculate_yoga_strength(self, positions: List[PlanetaryPosition]) -> float:
        ...
        for pos in positions:
            # Dignity score
            dignity = self.math.calculate_dignity_score(pos.body, pos.sign, pos.house)
    ...
    def _is_in_own_sign(self, body: CelestialBody, sign: ZodiacSign) -> bool:
        """Check if planet is in own sign"""
        from ..astronomical.config import ZODIAC_PROPERTIES

        return ZODIAC_PROPERTIES[sign]["ruler"] == body

    def _is_in_exaltation(self, body: CelestialBody, sign: ZodiacSign) -> bool:
    ...
    def _get_mahapurusha_effects(self, body: CelestialBody) -> List[str]:
        ...
        effects_map = {
            ...
            CelestialBody.MERCURY: ["Intelligence", "Business acumen", "Communication skills", "Scientific knowledge"],
            CelestialBody.JUPITER: ["Wisdom", "Spiritual advancement", "Teaching abilities", "Wealth and prosperity"],
            CelestialBody.VENUS: [
                "Artistic talents",
                "Luxury and comfort",
                "Diplomatic skills",
    ...
    def _calculate_dasha_strength(
        self, planet_pos: PlanetaryPosition, all_positions: Dict[CelestialBody, PlanetaryPosition]
    ) -> float:
        """Calculate dasha period strength"""
        # Base strength from dignity
        strength = self.math.calculate_dignity_score(planet_pos.body, planet_pos.sign, planet_pos.house)
    ...
    def _get_dasha_effects(self, body: CelestialBody) -> List[str]:
        ...
        effects_map = {
            ...
            CelestialBody.MOON: ["Emotional well-being", "Public life", "Mother-related matters", "Travel and changes"],
            CelestialBody.MARS: ["Energy and initiative", "Property matters", "Technical projects", "Competition"],
            CelestialBody.RAHU: ["Foreign influences", "Unconventional paths", "Sudden changes", "Material gains"],
            CelestialBody.JUPITER: [
            ...
            CelestialBody.SATURN: ["Career development", "Responsibility", "Delays and obstacles", "Long-term gains"],
            CelestialBody.MERCURY: ["Education and skills", "Business ventures", "Communication", "Short travels"],
            CelestialBody.KETU: ["Spiritual insights", "Liberation", "Technical skills", "Detachment"],
            CelestialBody.VENUS: [
                "Relationships",
                "Luxury and comfort",

...
from .transformer import TransformationType, data_transformer
from .validation import ValidationLevel, ValidationScope, data_validator
...
class DataPipeline:
    ...
    async def process(
        self, data: Union[Dict[str, Any], BaseModel], config: Optional[PipelineConfig] = None
    ) -> PipelineResult:
        ...
        try:
            ...
            transformation_result = await self._execute_transformation(data, pipeline_config.transformation_rules)
            ...
            return PipelineResult(
                ...
                stage=PipelineStage.VALIDATION,
                data=data,
                metadata={"error": str(e), "duration": (datetime.utcnow() - start_time).total_seconds()},
            )

    async def _execute_validation(
        self, data: Union[Dict[str, Any], BaseModel], config: PipelineConfig
    ) -> Dict[str, Any]:
        """Execute validation stage"""
        validation_result = await data_validator.validate(
            data, level=config.validation_level, scope=config.validation_scope
        )
        return validation_result.dict()

    async def _execute_transformation(self, data: Union[Dict[str, Any], BaseModel], rules: List[str]) -> Dict[str, Any]:
        ...
        for name, enricher in self._enrichers.items():
            try:
                ...
                if result["success"]:
                    ...
                    changes.append({"enricher": name, "timestamp": datetime.utcnow().isoformat()})
            except Exception as e:
                return {"success": False, "error": str(e), "data": data}

        return {"success": True, "data": enriched_data, "changes": changes}
    ...
    async def _execute_persistence(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for name, persister in self._persisters.items():
            try:
                ...
                results.append(
                    {"persister": name, "success": result["success"], "timestamp": datetime.utcnow().isoformat()}
                )
            except Exception as e:
                results.append(
                    {"persister": name, "success": False, "error": str(e), "timestamp": datetime.utcnow().isoformat()}
                )

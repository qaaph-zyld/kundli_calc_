...
class DataValidator:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        self.add_rule(
            ValidationRule(
                ...
                description="Validates date format",
                validation_type="regex",
                parameters={"pattern": r"^\d{4}-\d{2}-\d{2}$", "error_message": "Invalid date format. Use YYYY-MM-DD"},
                level=ValidationLevel.STRICT,
                scope=ValidationScope.INPUT,
            )
        )
        ...
        self.add_rule(
            ValidationRule(
                ...
                parameters={"min": -90, "max": 90, "error_message": "Invalid latitude. Must be between -90 and 90"},
                level=ValidationLevel.STRICT,
                scope=ValidationScope.INPUT,
            )
        )
    ...
    async def validate(
        ...
        errors = []
        warnings = []
        metadata = {"validation_level": level, "validation_scope": scope, "rules_applied": []}
        ...
        for rule in sorted_rules:
            try:
                ...
                errors.append({"rule": rule.name, "error": str(e), "field": None})

        return ValidationResult(valid=len(errors) == 0, errors=errors, warnings=warnings, metadata=metadata)

    async def _validate_regex(
        self, data: Dict[str, Any], rule: ValidationRule, errors: List[Dict[str, Any]], warnings: List[Dict[str, Any]]
    ) -> None:
        ...
        for field, value in data.items():
            if isinstance(value, str) and not re.match(pattern, value):
                errors.append(
                    {"rule": rule.name, "error": rule.parameters["error_message"], "field": field, "value": value}
                )

...
class DataTransformer:
    ...
    def _initialize_default_rules(self) -> None:
        ...
        self.add_rule(
            TransformationRule(
                ...
                parameters={"precision": 6, "format": "decimal"},
            )
        )
        ...
        self.add_rule(
            TransformationRule(
                ...
                transformation_type=TransformationType.FORMAT,
                parameters={"source_timezone": "UTC", "target_timezone": "local"},
            )
        )
    ...
    async def transform(
        self, data: Union[Dict[str, Any], BaseModel], rules: Optional[List[str]] = None
    ) -> TransformationResult:
        """Transform data using specified rules"""
        changes = []
        metadata = {"rules_applied": [], "start_time": datetime.utcnow()}
        ...
        try:
            ...
            for rule in rules_to_apply:
                ...
                # Record changes
                changes.append(
                    {"rule": rule.name, "type": rule.transformation_type, "timestamp": datetime.utcnow().isoformat()}
                )

            metadata["end_time"] = datetime.utcnow()
            metadata["duration"] = (metadata["end_time"] - metadata["start_time"]).total_seconds()

            return TransformationResult(success=True, data=transformed_data, changes=changes, metadata=metadata)
            ...
            return TransformationResult(
                ...
                data=data,
                changes=changes,
                metadata={**metadata, "error": str(e), "end_time": datetime.utcnow()},
            )
    ...
    async def _transform_datetime_to_iso8601(self, data: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _normalize_coordinates(self, data: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _transform_timezone(self, data: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:

...
class QueryOptimizer:
    ...
    async def ensure_indexes(self):
        ...
        try:
            ...
            kundli_indexes = [
                ...
                IndexModel(
                    [("date", ASCENDING), ("time", ASCENDING), ("latitude", ASCENDING), ("longitude", ASCENDING)]
                ),
    ...
    async def find_kundli(
        ...
        try:
            result = await self.db.kundlis.find_one(query, projection=projection)
            return result
        except Exception as e:
    ...
    async def find_kundlis(
        ...
        try:
            cursor = self.db.kundlis.find(query, projection=projection)
    ...
    async def aggregate_kundlis(self, pipeline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def update_kundli(self, query: Dict[str, Any], update: Dict[str, Any], upsert: bool = False) -> bool:
        """Optimized update query"""
        try:
            result = await self.db.kundlis.update_one(query, update, upsert=upsert)
    ...
    async def bulk_update_kundlis(self, operations: List[Dict[str, Any]]) -> bool:
    ...
    async def optimize_query(self, query: Dict[str, Any], collection: str) -> Dict[str, Any]:
        """Optimize query based on indexes and data distribution"""
        optimized = query.copy()

        try:
            # Get collection stats
            stats = await self.db.command({"collStats": collection, "scale": 1024})

            # Check if query uses indexes
            explain = await self.db[collection].find(optimized).explain()

            # If not using indexes and collection is large,
            # try to optimize
            if (
                stats.get("size", 0) > 100 * 1024 * 1024  # 100MB
                and explain.get("queryPlanner", {}).get("winningPlan", {}).get("stage") == "COLLSCAN"
            ):
...
QUERY_PATTERNS = {
    "recent_kundlis": {
        ...
        "sort": [("created_at", -1)],
        "projection": {"_id": 1, "date": 1, "time": 1, "calculation_type": 1},
    },
    "user_calculations": {
        "pipeline": [
            {"$match": {"user_id": "$user_id"}},
            {"$group": {"_id": "$calculation_type", "count": {"$sum": 1}, "last_calculation": {"$max": "$created_at"}}},
        ]
    },
}

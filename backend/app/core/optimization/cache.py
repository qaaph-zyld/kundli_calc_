...
class CacheConfig:
    ...
    # Cache prefixes
    PREFIXES = {"kundli": "kdl:", "planetary": "plt:", "transit": "trn:", "compatibility": "cmp:", "user": "usr:"}
...
class AsyncCache:
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None, max_size: Optional[int] = None) -> bool:
        """Set value in cache"""
        try:
            # Serialize value
            ...
            # Set with TTL
            await self.redis.set(key, serialized, ex=ttl if ttl is not None else self.config.TTL["default"])
            return True
        except Exception as e:
...
def cached(prefix: str, ttl: Optional[int] = None, max_size: Optional[int] = None):

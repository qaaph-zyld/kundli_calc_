...
class LogProcessor:
    ...
    def _add_extra_fields(self, logger: str, method_name: str, event_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def process_record(self, record: logging.LogRecord, context: Optional[LogContext] = None) -> str:
        """Process log record"""
...
class LogWriter:
    ...
    async def _write_elastic(self, log_entry: Dict[str, Any]) -> None:
        ...
        await self.elastic.index(index=self.config.elastic_index, document=log_entry)
    ...
    def _format_console(self, log_entry: Dict[str, Any]) -> str:
        ...
        context = {k: v for k, v in log_entry.items() if k not in ["level", "timestamp", "message"]}
...
class LogManager:
    ...
    def log(self, level: LogLevel, message: str, context: Optional[LogContext] = None, **kwargs: Any) -> None:

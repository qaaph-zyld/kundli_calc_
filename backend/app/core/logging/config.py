...
from .framework import LogConfig, LogDestination, LogFormat, LogLevel
...
def get_logging_config(environment: str) -> LogConfig:
    ...
    # Base configuration
    base_config = {
        "level": LogLevel.DEBUG if environment in ["local", "development"] else LogLevel.INFO,
        "format": LogFormat.CONSOLE if environment == "local" else LogFormat.JSON,
    ...
    if environment == "local":
        config = {**base_config, "destinations": [LogDestination.CONSOLE], "file_path": None}
        ...
        config = {
            **base_config,
            "destinations": [LogDestination.FILE, LogDestination.ELASTIC],
        ...
        config = {
            **base_config,
            "destinations": [LogDestination.FILE, LogDestination.ELASTIC, LogDestination.SYSLOG],

...
class CareerSynthesisEngine:
    ...
    def synthesize_career_analysis(
        ...
        tenth_house_factor = self._analyze_career_house(chart_data, 10, "Career & Status", current_dasha)
        ...
        sixth_house_factor = self._analyze_career_house(chart_data, 6, "Service & Competition", current_dasha)
        ...
        second_house_factor = self._analyze_career_house(chart_data, 2, "Income & Resources", current_dasha)
        ...
        sun_factor = self._analyze_career_karaka(chart_data, "Sun", "Authority & Leadership", current_dasha)
        if sun_factor:
        ...
        saturn_factor = self._analyze_career_karaka(chart_data, "Saturn", "Service & Discipline", current_dasha)
        if saturn_factor:
        ...
        yoga_factors = self._analyze_career_yogas(chart_data.get("active_yogas", []))
        ...
        timing_info = self._generate_career_timing(chart_data, factors, current_dasha)

        # Generate synthesis narrative
        synthesis_text = self._generate_career_synthesis(factors, normalized_score, strength_level, timing_info)

        # Generate overall assessment
        assessment = self._generate_overall_assessment(normalized_score, strength_level, factors)

        # Generate recommendations
        recommendations = self._generate_career_recommendations(factors, strength_level, timing_info)
    ...
    def _analyze_career_yogas(self, active_yogas: List[str]) -> List[CareerFactor]:
        """Analyze career-relevant yogas"""
        yoga_factors = []

        for yoga_name in active_yogas:
            # Check if yoga is career-relevant
            is_career_yoga = any(keyword in yoga_name for keyword in self.career_yoga_keywords)
    ...
    def _generate_career_timing(
        self, chart_data: Dict[str, Any], factors: List[CareerFactor], current_dasha: Optional[str]
    ) -> Dict[str, Any]:
        """Generate timing analysis for career"""
        timing = {"current_period": "", "peak_periods": [], "favorable_transits": [], "career_change_windows": []}
    ...
    def _generate_career_synthesis(
        self, factors: List[CareerFactor], score: float, strength: CareerStrength, timing: Dict[str, Any]
    ) -> str:
    ...
    def _generate_overall_assessment(self, score: float, strength: CareerStrength, factors: List[CareerFactor]) -> str:
    ...
    def _generate_career_recommendations(
        self, factors: List[CareerFactor], strength: CareerStrength, timing: Dict[str, Any]
    ) -> List[str]:
        ...
        if strength in [CareerStrength.EXCEPTIONAL, CareerStrength.VERY_STRONG]:
            recommendations.append("Pursue leadership and high-responsibility roles - chart supports authority")
            recommendations.append("Consider entrepreneurship or independent professional practice")
        elif strength == CareerStrength.STRONG:
            recommendations.append("Focus on steady career advancement through demonstrated competence")
            recommendations.append("Build professional network and seek mentorship")
        else:
            recommendations.append("Develop specialized skills to overcome career obstacles")
            recommendations.append("Consider service-oriented or technical professions")

        # Timing-based recommendations
        if timing.get("peak_periods"):
            recommendations.append(f"Plan major career moves during: {timing['peak_periods'][0]}")

        # Factor-specific recommendations
        for factor in factors:
            if "10th" in factor.factor_name and factor.strength_level == "strong":
                recommendations.append("Leverage strong 10th house for public-facing roles")
                break

        return recommendations[:5]  # Top 5 recommendations

    def _calculate_confidence(self, factors: List[CareerFactor], sources_count: int) -> float:

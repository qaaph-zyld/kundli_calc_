...
from app.core.knowledge.engine.contextual_synthesis_engine import ContextualSynthesisEngine, StrengthLevel
...
class WealthSynthesisEngine:
    ...
    def __init__(self):
        ...
        self.wealth_yoga_keywords = ["Dhana", "Lakshmi", "Wealth", "Prosperity", "Gaja_Kesari"]

    def synthesize_wealth_analysis(
        self, chart_data: Dict[str, Any], current_dasha: Optional[str] = None
    ) -> WealthSynthesis:
        ...
        second_factor = self._analyze_wealth_house(chart_data, 2, "Family Wealth & Assets", current_dasha)
        ...
        eleventh_factor = self._analyze_wealth_house(chart_data, 11, "Gains & Income", current_dasha)
        ...
        fifth_factor = self._analyze_wealth_house(chart_data, 5, "Speculation & Investments", current_dasha)
        ...
        ninth_factor = self._analyze_wealth_house(chart_data, 9, "Fortune & Inheritance", current_dasha)
        ...
        jupiter_factor = self._analyze_wealth_karaka(chart_data, "Jupiter", "Prosperity & Expansion", current_dasha)
        if jupiter_factor:
        ...
        yoga_factors = self._analyze_wealth_yogas(chart_data.get("active_yogas", []))
        ...
        timing_info = self._generate_wealth_timing(chart_data, factors, current_dasha)

        # Generate synthesis
        synthesis_text = self._generate_wealth_synthesis(factors, normalized_score, strength_level, timing_info)

        # Generate assessment
        assessment = self._generate_overall_assessment(normalized_score, strength_level, factors)

        # Generate recommendations
        recommendations = self._generate_wealth_recommendations(factors, strength_level, timing_info)
    ...
    def _analyze_wealth_house(
        self, chart_data: Dict[str, Any], house_num: int, house_theme: str, current_dasha: Optional[str]
    ) -> WealthFactor:
        """Analyze wealth-relevant house"""

        planets_in_house = [p for p, data in chart_data.get("planets", {}).items() if data.get("house") == house_num]
        ...
        if house_lord:
            ...
            if lord_house in [1, 2, 4, 5, 7, 9, 10, 11]:
                score += 5
                interpretation_parts.append(f"{house_num}th lord {house_lord} in {lord_house}th (favorable)")
            elif lord_house in [6, 8, 12]:
                score -= 3
                interpretation_parts.append(f"{house_num}th lord {house_lord} in {lord_house}th (challenging)")

        final_score = max(0, min(base_score, base_score + score))
        strength = (
            "strong" if final_score > base_score * 0.7 else "moderate" if final_score > base_score * 0.4 else "weak"
        )
    ...
    def _analyze_wealth_karaka(
        self, chart_data: Dict[str, Any], planet: str, karaka_theme: str, current_dasha: Optional[str]
        ...
        result = self.contextual_engine.synthesize_interpretation(
            planet=planet, house=house, sign=sign, dignity=dignity, current_dasha=current_dasha
        )
    ...
    def _analyze_wealth_yogas(self, active_yogas: List[str]) -> List[WealthFactor]:
        """Analyze Dhana yogas (wealth-forming combinations)"""

        yoga_factors = []

        for yoga_name in active_yogas:
            is_wealth_yoga = any(keyword in yoga_name for keyword in self.wealth_yoga_keywords)
    ...
    def _generate_wealth_timing(
        self, chart_data: Dict[str, Any], factors: List[WealthFactor], current_dasha: Optional[str]
    ) -> Dict[str, Any]:
        """Generate timing analysis for wealth"""

        timing = {"current_period": "", "wealth_gain_windows": [], "favorable_periods": []}
        ...
        if second_lord:
            timing["wealth_gain_windows"].append(f"{second_lord} mahadasha (2nd lord period - family wealth)")

        eleventh_lord = chart_data.get("house_lords", {}).get(11)
        if eleventh_lord:
            timing["wealth_gain_windows"].append(f"{eleventh_lord} mahadasha (11th lord period - gains)")

        jupiter_data = chart_data.get("planets", {}).get("Jupiter", {})
        if jupiter_data:
            timing["wealth_gain_windows"].append("Jupiter mahadasha (Dhana Karaka period)")

        return timing

    def _generate_wealth_synthesis(
        self, factors: List[WealthFactor], score: float, strength: WealthStrength, timing: Dict[str, Any]
    ) -> str:
        ...
        for factor in factors[:5]:
            parts.append(
                f"\n• {factor.factor_name} ({factor.contribution_score} points): " f"{factor.interpretation[:100]}..."
            )
    ...
    def _generate_overall_assessment(self, score: float, strength: WealthStrength, factors: List[WealthFactor]) -> str:
    ...
    def _generate_wealth_recommendations(
        self, factors: List[WealthFactor], strength: WealthStrength, timing: Dict[str, Any]
    ) -> List[str]:
        ...
        if strength in [WealthStrength.EXCEPTIONAL, WealthStrength.VERY_STRONG]:
            recommendations.append("Pursue wealth-building opportunities during favorable dasha periods")
            recommendations.append("Consider investments and business ventures - chart supports financial growth")
            recommendations.append("Leverage strong Jupiter/wealth houses for prosperity")
        elif strength == WealthStrength.STRONG:
            recommendations.append("Focus on steady wealth accumulation through savings and investments")
            recommendations.append("Build multiple income streams for financial security")
        else:
            recommendations.append("Prioritize financial discipline and careful budgeting")
            recommendations.append("Avoid speculation - focus on stable income sources")
            recommendations.append("Strengthen Jupiter through remedial measures for wealth")

        if timing.get("wealth_gain_windows"):
            recommendations.append(f"Optimal wealth timing: {timing['wealth_gain_windows'][0]}")

        return recommendations[:5]

    def _calculate_confidence(self, factors: List[WealthFactor], sources_count: int) -> float:

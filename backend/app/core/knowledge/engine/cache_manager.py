...
class InterpretationCache:
    ...
    def _generate_key(
        self, planet: str, house: int, sign: str = "", dignity: str = "", multi_source: bool = False
    ) -> str:
        """
        ...
        """
        key_data = {"planet": planet, "house": house, "sign": sign, "dignity": dignity, "multi_source": multi_source}
    ...
    def _get_most_accessed(self, limit: int = 5) -> list:
        ...
        sorted_access = sorted(self.access_counts.items(), key=lambda x: x[1], reverse=True)
        return [{"key": key[:8], "access_count": count} for key, count in sorted_access[:limit]]
        ...
        for operation, times in self.operation_times.items():
            if times:
                stats[operation] = {
                    ...
                    "recent_avg_ms": (
                        round(sum(times[-10:]) / len(times[-10:]) * 1000, 2)
                        if len(times) >= 10
                        else round(sum(times) / len(times) * 1000, 2)
                    ),
                }

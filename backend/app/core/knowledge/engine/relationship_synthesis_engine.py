...
class RelationshipSynthesisEngine:
    ...
    def __init__(self):
        ...
        self.relationship_yoga_keywords = ["Marriage", "Kalatr", "Venus", "Shubha", "Parivartana"]
    ...
    def synthesize_relationship_analysis(
        ...
        seventh_factor = self._analyze_relationship_house(chart_data, 7, "Marriage & Partnership", current_dasha)
        ...
        fifth_factor = self._analyze_relationship_house(chart_data, 5, "Romance & Love", current_dasha)
        ...
        eighth_factor = self._analyze_relationship_house(chart_data, 8, "Intimacy & Transformation", current_dasha)
        ...
        venus_factor = self._analyze_relationship_karaka(chart_data, "Venus", "Love & Harmony", current_dasha)
        if venus_factor:
        ...
        yoga_factors = self._analyze_relationship_yogas(chart_data.get("active_yogas", []))
        ...
        timing_info = self._generate_relationship_timing(chart_data, factors, current_dasha)

        # Generate synthesis
        synthesis_text = self._generate_relationship_synthesis(factors, normalized_score, strength_level, timing_info)

        # Generate assessment
        assessment = self._generate_overall_assessment(normalized_score, strength_level, factors)

        # Generate recommendations
        recommendations = self._generate_relationship_recommendations(factors, strength_level, timing_info)
    ...
    def _analyze_relationship_yogas(self, active_yogas: List[str]) -> List[RelationshipFactor]:
        """Analyze relationship-relevant yogas"""

        yoga_factors = []

        for yoga_name in active_yogas:
            is_relationship_yoga = any(keyword in yoga_name for keyword in self.relationship_yoga_keywords)
    ...
    def _generate_relationship_timing(
        self, chart_data: Dict[str, Any], factors: List[RelationshipFactor], current_dasha: Optional[str]
    ) -> Dict[str, Any]:
        """Generate timing analysis for relationships"""

        timing = {"current_period": "", "marriage_windows": [], "favorable_periods": []}
        ...
        if seventh_lord:
            timing["marriage_windows"].append(f"{seventh_lord} mahadasha (7th lord period)")
    ...
    def _generate_relationship_synthesis(
        self, factors: List[RelationshipFactor], score: float, strength: RelationshipStrength, timing: Dict[str, Any]
    ) -> str:
    ...
    def _generate_overall_assessment(
        self, score: float, strength: RelationshipStrength, factors: List[RelationshipFactor]
    ) -> str:
    ...
    def _generate_relationship_recommendations(
        self, factors: List[RelationshipFactor], strength: RelationshipStrength, timing: Dict[str, Any]
    ) -> List[str]:
        ...
        if strength in [RelationshipStrength.EXCEPTIONAL, RelationshipStrength.VERY_STRONG]:
            recommendations.append("Pursue relationships during favorable dasha periods for best results")
            recommendations.append("Strong indicators for harmonious marriage - trust natural timing")
        elif strength == RelationshipStrength.STRONG:
            recommendations.append("Focus on compatibility and shared values in partner selection")
            recommendations.append("Build strong communication foundation before commitment")
        else:
            recommendations.append("Careful partner selection crucial - seek astrological compatibility")
            recommendations.append("Consider relationship counseling or pre-marital guidance")
            recommendations.append("Strengthen Venus through remedial measures")

        if timing.get("marriage_windows"):
            recommendations.append(f"Optimal marriage timing: {timing['marriage_windows'][0]}")

        return recommendations[:5]

    def _calculate_confidence(self, factors: List[RelationshipFactor], sources_count: int) -> float:

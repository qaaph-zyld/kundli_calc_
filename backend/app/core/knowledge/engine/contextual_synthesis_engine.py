...
class ContextualSynthesisEngine:
    ...
    def synthesize_interpretation(
        ...
        base = self.knowledge_engine.interpret_planet_in_house(planet=planet, house=house, sign=sign, dignity=dignity)
        ...
        strength = self._assess_strength(planet, house, sign, dignity, lordship_houses, aspects, active_yogas)
        ...
        key_themes = self._extract_key_themes(base_dict, lordship_effects, aspect_effects, yoga_effects)

        # 10. Generate timing notes
        timing_notes = self._generate_timing_notes(planet, dasha_modulation, yoga_effects)

        # Calculate overall confidence
        confidence = self._calculate_confidence(
            base_dict, source_comparison, lordship_effects, aspect_effects, yoga_effects
        )
    ...
    def _analyze_lordship(self, planet: str, current_house: int, lordship_houses: List[int]) -> Dict[str, Any]:
        """Analyze effects of planetary lordship"""
        effects = {"ruling_houses": lordship_houses, "house_connections": [], "strength_note": ""}
    ...
    def _analyze_aspects(self, planet: str, house: int, aspects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _analyze_yogas(self, planet: str, active_yogas: List[str]) -> List[Dict[str, Any]]:
    ...
    def _analyze_dasha_effect(self, planet: str, current_dasha: str, house: int) -> Dict[str, Any]:
        """Analyze how current dasha modulates interpretation"""
        modulation = {"current_dasha": current_dasha, "relationship": "", "timing_note": ""}
    ...
    def _synthesize_all_factors(
        ...
        if lordship:
            synthesis_parts.append(
                f"\n\nLordship Context: {lordship['strength_note']}. " f"{' '.join(lordship['house_connections'])}"
            )
    ...
    def _generate_timing_notes(
        self, planet: str, dasha: Optional[Dict[str, Any]], yogas: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        if yogas:
            notes.append(f"Yoga effects manifest primarily during {planet} dasha period " f"and sub-periods")

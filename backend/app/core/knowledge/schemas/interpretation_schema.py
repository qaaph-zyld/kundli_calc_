...
from .source_schema import InterpretationSource, InterpretationMetadata
...
class PlanetInHouseInterpretation(BaseModel):
    ...
    life_areas: Dict[LifeArea, str] = Field(
        default_factory=dict,
        description="Effects on specific life areas"
    )
    ...
    timing_notes: Optional[str] = Field(
        None,
        description="When effects manifest (dashas, transits)"
    )
...
class DashaInterpretation(BaseModel):
    ...
    life_areas_activated: List[LifeArea] = Field(
        default_factory=list,
        description="Life areas most affected"
    )
    ...
    effects_by_house_placement: Optional[str] = Field(
        None,
        description="Effects based on planet's house position in chart"
    )
    
    # Recommendations
    recommendations: List[str] = Field(
        default_factory=list,
        description="How to navigate this period"
    )
...
class SynthesizedInterpretation(BaseModel):
    ...
    confidence_notes: str = Field(
        ...,
        description="Notes on interpretation confidence and any contradictions"
    )
    
    # Source synthesis
    sources_used: List[str] = Field(
        ...,
        description="All sources consulted for this synthesis"
    )
    synthesis_method: str = Field(
        ...,
        description="How multiple sources were combined"
    )

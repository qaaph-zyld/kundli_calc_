...
class MessageQueue:
    ...
    async def initialize(self, connection_url: str):
        ...
        for queue_name in self.queues.values():
            await self.channel.declare_queue(
                queue_name,
                durable=True,
                arguments={"x-max-priority": 10, "x-message-ttl": 3600000, "x-dead-letter-exchange": "dlx"},  # 1 hour
            )
    ...
    async def publish_message(self, queue_type: str, message: QueueMessage):
        ...
        await self.channel.default_exchange.publish(
            aio_pika.Message(
                body=message_body, priority=message.priority, delivery_mode=aio_pika.DeliveryMode.PERSISTENT
            ),

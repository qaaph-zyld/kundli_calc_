...
class TestFramework:
    ...
    async def run_suite(self, suite: TestSuite) -> List[TestResult]:
        ...
        try:
            ...
            if self.config.parallel:
                results = await asyncio.gather(*[self.run_case(case) for case in suite.cases])
            else:
    ...
    async def run_case(self, case: TestCase) -> TestResult:
        ...
        try:
            ...
            for attempt in range(self.config.retries):
                try:
                    ...
                    if self._validate_output(output, case.expected):
                        return TestResult(case=case, status=TestStatus.PASSED, duration=duration, output=output)
                    else:
                        return TestResult(
                            case=case,
            ...
            duration = (datetime.utcnow() - start_time).total_seconds()
            return TestResult(case=case, status=TestStatus.ERROR, duration=duration, error=str(e))
    ...
    async def _run_unit_test(self, case: TestCase, client: AsyncClient) -> Dict[str, Any]:
        """Run unit test"""
    ...
    async def _run_integration_test(self, case: TestCase, client: AsyncClient) -> Dict[str, Any]:
    ...
    async def _run_e2e_test(self, case: TestCase, client: AsyncClient) -> Dict[str, Any]:
        """Run end-to-end test"""
        results = {}
    ...
    async def _run_performance_test(self, case: TestCase, client: AsyncClient) -> Dict[str, Any]:
        ...
        for request in requests:
            ...
            try:
                ...
                results["summary"]["min_response_time"] = min(results["summary"]["min_response_time"], duration)
                results["summary"]["max_response_time"] = max(results["summary"]["max_response_time"], duration)
        ...
        if results["summary"]["successful_requests"] > 0:
            results["summary"]["avg_response_time"] = total_time / results["summary"]["successful_requests"]

        return results

    async def _run_security_test(self, case: TestCase, client: AsyncClient) -> Dict[str, Any]:
        """Run security test"""
        results = {}
    ...
    def _check_security_headers(self, headers: Dict[str, str]) -> bool:
        ...
        required_headers = {"X-Content-Type-Options", "X-Frame-Options", "X-XSS-Protection", "Content-Security-Policy"}
...
# Test decorators
def unit_test(name: str, priority: TestPriority = TestPriority.MEDIUM, tags: List[str] = None):
    """Decorator for unit tests"""

    def decorator(func: Callable):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            case = TestCase(name=name, scope=TestScope.UNIT, priority=priority, tags=tags or [], inputs=kwargs)
...
def integration_test(name: str, priority: TestPriority = TestPriority.MEDIUM, tags: List[str] = None):
    """Decorator for integration tests"""

    def decorator(func: Callable):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            case = TestCase(name=name, scope=TestScope.INTEGRATION, priority=priority, tags=tags or [], inputs=kwargs)
            return await test_framework.run_case(case)
...
def e2e_test(name: str, priority: TestPriority = TestPriority.HIGH, tags: List[str] = None):
    """Decorator for end-to-end tests"""

    def decorator(func: Callable):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            case = TestCase(name=name, scope=TestScope.E2E, priority=priority, tags=tags or [], inputs=kwargs)
            return await test_framework.run_case(case)
...
def performance_test(name: str, priority: TestPriority = TestPriority.MEDIUM, tags: List[str] = None):
    """Decorator for performance tests"""

    def decorator(func: Callable):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            case = TestCase(name=name, scope=TestScope.PERFORMANCE, priority=priority, tags=tags or [], inputs=kwargs)
            return await test_framework.run_case(case)
...
def security_test(name: str, priority: TestPriority = TestPriority.CRITICAL, tags: List[str] = None):
    """Decorator for security tests"""

    def decorator(func: Callable):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            case = TestCase(name=name, scope=TestScope.SECURITY, priority=priority, tags=tags or [], inputs=kwargs)
            return await test_framework.run_case(case)

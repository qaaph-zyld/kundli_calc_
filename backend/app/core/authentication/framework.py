...
class AuthenticationManager:
    ...
    def __init__(
        ...
        self.metrics = AuthenticationMetrics(
            login_attempts=0, failed_attempts=0, active_sessions=0, token_refreshes=0, average_session_duration=0.0
        )
    ...
    async def get_current_user(self, token: str = Depends(oauth2_scheme)) -> User:
        ...
        try:
            ...
            token_data = TokenData(username=username, scopes=payload.get("scopes", []), exp=payload.get("exp"))
    ...
    def login(self, form_data: OAuth2PasswordRequestForm) -> Token:
        ...
        # Create refresh token
        refresh_token = self.create_refresh_token(data={"sub": user.username, "scopes": user.scopes})
    ...
    def refresh_token(self, refresh_token: str) -> Token:
        ...
        try:
            ...
            return Token(access_token=access_token, token_type="bearer", expires_in=self.access_token_expire * 60)

        except jwt.PyJWTError:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid refresh token")

    def logout(self, current_user: User = Depends(get_current_active_user)):
        """Logout user"""
        # Update metrics
        self.metrics.active_sessions -= 1

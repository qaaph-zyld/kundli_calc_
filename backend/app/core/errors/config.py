...
from .framework import ErrorCategory, ErrorCode, ErrorResponse, ErrorSeverity
...
ERROR_SUGGESTIONS: Dict[ErrorCode, List[str]] = {
    ...
    ErrorCode.MISSING_FIELD: ["Provide all required fields", "Check the API documentation for required fields"],
    # Authentication errors
    ErrorCode.INVALID_CREDENTIALS: ["Verify your credentials", "Reset your password if needed"],
    ErrorCode.TOKEN_EXPIRED: ["Please log in again", "Refresh your authentication token"],
    # Authorization errors
    ErrorCode.UNAUTHORIZED: ["Log in to access this resource", "Check your account permissions"],
    ErrorCode.INSUFFICIENT_PERMISSIONS: ["Request elevated permissions", "Contact your administrator"],
    # Business logic errors
    ErrorCode.RESOURCE_NOT_FOUND: ["Verify the resource identifier", "Check if the resource exists"],
    ...
    # System errors
    ErrorCode.INTERNAL_ERROR: ["Try again later", "Contact support if the issue persists"],
    ErrorCode.SERVICE_UNAVAILABLE: ["Wait for service restoration", "Check service status page"],
    # Database errors
    ErrorCode.DATABASE_ERROR: ["Try again later", "Check database connection", "Verify data integrity"],
    # Cache errors
    ErrorCode.CACHE_ERROR: ["Try again later", "Clear your cache", "Refresh the page"],
    # Network errors

...
class ValidationError(AppError):
    ...
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None, context: Optional[ErrorContext] = None):
        super().__init__(
...
class AuthenticationError(AppError):
    ...
    def __init__(
        self, message: str, code: ErrorCode = ErrorCode.INVALID_CREDENTIALS, context: Optional[ErrorContext] = None
    ):
        super().__init__(
            code=code,
            message=message,
...
class AuthorizationError(AppError):
    ...
    def __init__(self, message: str, code: ErrorCode = ErrorCode.UNAUTHORIZED, context: Optional[ErrorContext] = None):
        super().__init__(
            code=code,
            message=message,
...
class SystemError(AppError):
    ...
    def __init__(
        self, message: str, code: ErrorCode = ErrorCode.INTERNAL_ERROR, context: Optional[ErrorContext] = None
    ):
        super().__init__(
            code=code,
            message=message,
...
class ExternalServiceError(AppError):
    ...
    def __init__(
        self, message: str, code: ErrorCode = ErrorCode.EXTERNAL_SERVICE_ERROR, context: Optional[ErrorContext] = None
    ):
        super().__init__(
            code=code,
            message=message,
...
class ErrorHandler:
    ...
    def _create_error_context(
        self, request: Optional[Request] = None, exc_info: Optional[tuple] = None
    ) -> ErrorContext:
    ...
    async def _handle_validation_error(self, request: Request, exc: RequestValidationError) -> JSONResponse:
        ...
        error = ValidationError(message="Invalid input data", details={"errors": exc.errors()}, context=context)
        return JSONResponse(status_code=error.status_code, content=error.to_response().dict())

    async def _handle_http_error(self, request: Request, exc: StarletteHTTPException) -> JSONResponse:
        """Handle HTTP error"""
        context = self._create_error_context(request)
        error = AppError(
    ...
    async def _handle_app_error(self, request: Request, exc: AppError) -> JSONResponse:
        ...
        context = self._create_error_context(request)
        exc.context = context
        return JSONResponse(status_code=exc.status_code, content=exc.to_response().dict())
    ...
    async def _handle_unknown_error(self, request: Request, exc: Exception) -> JSONResponse:
        ...
        error = SystemError(message="An unexpected error occurred", context=context)
        return JSONResponse(status_code=error.status_code, content=error.to_response().dict())

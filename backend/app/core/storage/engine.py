...
class StorageEngine:
    ...
    async def _initialize_mongodb(self) -> None:
        ...
        self._client = motor.motor_asyncio.AsyncIOMotorClient(self.config.connection_string, **self.config.options)
    ...
    async def find_one(
        self, query: StorageQuery, model: Optional[Type[BaseModel]] = None
        ...
        if self.config.type == StorageType.MONGODB:
            ...
            document = await self._collection.find_one(
                mongo_query, projection=dict.fromkeys(query.projection, 1) if query.projection else None
            )
    ...
    async def update_one(self, query: StorageQuery, update: Dict[str, Any]) -> bool:
    ...
    async def update_many(self, query: StorageQuery, update: Dict[str, Any]) -> int:

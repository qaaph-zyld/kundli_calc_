...
from .engine import QueryOperator, StorageEngine, StorageQuery, kundli_storage, pattern_storage
...
class Repository(Generic[T]):
    ...
    async def find_by_id(self, id: str) -> Optional[T]:
        """Find entity by ID"""
        query = StorageQuery(filters={"_id": {QueryOperator.EQ: id}})

        return await self.storage.find_one(query, self.model)

    async def find_one(self, filters: Dict[str, Dict[str, Any]], projection: Optional[List[str]] = None) -> Optional[T]:
        """Find single entity by filters"""
        query = StorageQuery(filters=filters, projection=projection)
    ...
    async def find_many(
        ...
        query = StorageQuery(filters=filters or {}, sort=sort, skip=skip, limit=limit, projection=projection)
    ...
    async def update_many(self, filters: Dict[str, Dict[str, Any]], data: Dict[str, Any]) -> int:
...
kundli_repository = Repository[KundliData](storage=kundli_storage, model=KundliData)

pattern_repository = Repository[PatternData](storage=pattern_storage, model=PatternData)

...
class GocharaSystem:
    ...
    def analyze_transit(
        ...
        # Check for Vedha
        has_vedha, vedha_planet = self._check_vedha(
            planet, house_from_ref, transit_positions, ref_pos
        )
        
        # Determine strength (combining traditional + Ashtakavarga)
        strength = self._calculate_transit_strength(
            is_benefic, av_bindus, has_vedha
        )
        
        # Generate interpretation
        interpretation = self._interpret_transit(
            planet, house_from_ref, is_benefic, strength, 
            has_vedha, vedha_planet, reference_point
        )
        ...
        for planet in self.planets:
            if planet in transit_positions:
                results_from_moon[planet] = self.analyze_transit(
                    planet, natal_positions, transit_positions, ascendant,
                    ashtakavarga_bindus, reference_point='Moon'
                )
                
                results_from_lagna[planet] = self.analyze_transit(
                    planet, natal_positions, transit_positions, ascendant,
                    ashtakavarga_bindus, reference_point='Lagna'
                )
        
        # Identify key transits (benefic with high AV, or malefic with vedha)
        key_transits = []
        for planet, result in results_from_moon.items():
            if (result.strength in [TransitStrength.EXCELLENT, TransitStrength.VERY_GOOD] or
                result.has_vedha or
                result.strength == TransitStrength.VERY_DIFFICULT):
                key_transits.append({
                    'planet': planet,
    ...
    def _overall_transit_assessment(
        self,
        results: Dict[str, TransitResult]
    ) -> str:
        """Provide overall assessment of current transit period"""
        excellent_count = sum(1 for r in results.values() 
                            if r.strength == TransitStrength.EXCELLENT)
        good_count = sum(1 for r in results.values() 
                        if r.strength in [TransitStrength.VERY_GOOD, TransitStrength.GOOD])
        difficult_count = sum(1 for r in results.values() 
                            if r.strength in [TransitStrength.DIFFICULT, TransitStrength.VERY_DIFFICULT])

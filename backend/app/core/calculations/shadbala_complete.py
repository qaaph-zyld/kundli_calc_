...
class CompleteShadbalaCalculator:
    ...
    def calculate_complete_shadbala(
        ...
        # Convert to sidereal
        sidereal_positions = {planet: (lon - ayanamsa) % 360 for planet, lon in planet_positions.items()}
    ...
    def _calculate_planet_shadbala(
        ...
        total_shashtiamsas = sthana_bala + dig_bala + kala_bala + chesta_bala + naisargika_bala + drik_bala
    ...
    def _calculate_sthana_bala(self, planet: str, longitude: float, all_positions: Dict[str, float]) -> float:
        """
    ...
    def _calculate_dig_bala(self, planet: str, house: int) -> float:
        ...
        if house == best_house:
            ...
            dist_from_best = min(abs(house - best_house), 12 - abs(house - best_house))
            # Linear interpolation
            return 60.0 * (1.0 - dist_from_best / 6.0)

    def _calculate_kala_bala(self, planet: str, birth_datetime: datetime, longitude: float, is_day: bool) -> float:
        """
    ...
    def _calculate_chesta_bala(self, planet: str, speed: float) -> float:
        ...
        avg_speeds = {"Mars": 0.5, "Mercury": 1.2, "Jupiter": 0.15, "Venus": 1.0, "Saturn": 0.1}
    ...
    def _calculate_drik_bala(self, planet: str, longitude: float, all_positions: Dict[str, float]) -> float:
        """
    ...
    def _is_daytime(self, birth_datetime: datetime, latitude: float, longitude: float) -> bool:
        ...
        jd = swe.julday(birth_datetime.year, birth_datetime.month, birth_datetime.day, 0)

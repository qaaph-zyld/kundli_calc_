    ...
    def rectify(
        ...
        return RectificationResult(
            ...
            details={
                ...
                "top_results": [
                    {"time": bt[0].isoformat(), "rp": bt[1], "analysis": bt[2]}
                    for bt in best_times[:3]
                ]
            }
        )
...
class BirthTimeRectifier:
    ...
    def full_rectification(
        ...
        # 1. Tattwa Shuddhi
        tattwa_result = self.tattwa.rectify(
            approximate_time, sunrise, moon_sign, asc_sign, tolerance_minutes
        )
        ...
        # 2. KP Ruling Planets
        kp_result = self.kp_rp.rectify(
            approximate_time, moon_longitude, latitude, longitude, tolerance_minutes
        )
        ...
        if events:
            event_result = self.event_based.rectify_using_events(
                approximate_time, events, tolerance_minutes
            )
            results["event_based"] = {
        ...
        # 5. Find consensus
        all_suggestions = (
            tattwa_result.suggested_times + 
            kp_result.suggested_times
        )

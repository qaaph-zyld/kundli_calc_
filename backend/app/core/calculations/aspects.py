...
class EnhancedAspectCalculator:
    ...
    def _calculate_orb_strength(self, aspect: Aspect, orb: float) -> float:
        """Calculate strength based on orb"""
        if orb == 0:
            return 100
        return max(0, 100 - (orb / aspect.orb) * 100)

    def _calculate_speed_strength(self, planet1_speed: float, planet2_speed: float) -> float:
    ...
    def _calculate_dignity_strength(self, planet1_dignity: str, planet2_dignity: str) -> float:
        ...
        return (dignity_values.get(planet1_dignity, 30) + dignity_values.get(planet2_dignity, 30)) / 2

    def _calculate_relationship_strength(self, planet1: str, planet2: str) -> float:
    ...
    def _calculate_house_position_strength(self, house1: int, house2: int) -> float:
    ...
    def calculate_planet_aspects(self, planet_positions: Dict[str, float], orb: float) -> List[Dict[str, Any]]:
        ...
        for i, planet1 in enumerate(planets):
            for planet2 in planets[i + 1 :]:
                angle = self.calculate_aspect_angle(planet_positions[planet1], planet_positions[planet2])
    ...
    def _is_applying(planet1: Dict[str, Any], planet2: Dict[str, Any]) -> bool:
        ...
        relative_speed = planet1.get("speed", 0) - planet2.get("speed", 0)
    ...
    def calculate_aspect_strength(
        self, aspect: Aspect, planet1: Dict[str, Any], planet2: Dict[str, Any], orb: float
    ) -> float:
        ...
        strengths.append(self._calculate_speed_strength(planet1.get("speed", 0), planet2.get("speed", 0)))
        strengths.append(
            self._calculate_dignity_strength(planet1.get("dignity", "neutral"), planet2.get("dignity", "neutral"))
        )
        strengths.append(self._calculate_relationship_strength(planet1["name"], planet2["name"]))
        strengths.append(self._calculate_house_position_strength(planet1.get("house", 1), planet2.get("house", 1)))
    ...
    def calculate_total_influence(self, aspect: Aspect, strength: float, is_applying: bool) -> float:
    ...
    def calculate_aspects(self, planetary_positions: Dict[str, Dict[str, Any]]) -> List[PlanetaryAspect]:
                ...
                for aspect in self.ASPECTS.values():
                    if self.is_aspect_within_orb(aspect.angle, diff, aspect.orb):
                        ...
                        strength = self.calculate_aspect_strength(
                            aspect, {"name": planet1, **p1_data}, {"name": planet2, **p2_data}, orb
                        )

                        # Calculate total influence
                        total_influence = self.calculate_total_influence(aspect, strength, is_applying)
    ...
    def calculate_special_aspects(
        self, planet: str, house: int, special_aspects: Optional[List[int]] = None
    ) -> List[int]:

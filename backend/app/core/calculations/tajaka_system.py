...
class TajakaCalculator:
    ...
    # Tajaka aspect orbs
    ASPECT_ORBS = {0: 12, 60: 6, 90: 8, 120: 8, 180: 12}  # Conjunction  # Sextile  # Square  # Trine  # Opposition
    ...
    def calculate_annual_chart(
        ...
        return_date = datetime(
            birth_date.year + year_number, birth_date.month, birth_date.day, birth_date.hour, birth_date.minute
        )
        ...
        return {
            ...
            "ascendant": {"longitude": annual_asc, "sign": SIGNS[int(annual_asc / 30)], "degree": annual_asc % 30},
            "muntha": {"sign": SIGNS[muntha], "house": ((muntha - int(annual_asc / 30)) % 12) + 1},
            "year_lord": year_lord,
            "planets": planets,
            "strengths": strengths,
    ...
    def _calculate_annual_positions(self, natal_sun: float, return_date: datetime) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            planet: {"longitude": lon, "sign": SIGNS[int(lon / 30)], "degree": lon % 30}
            for planet, lon in positions.items()
        }

    def _calculate_annual_ascendant(self, time: datetime, latitude: float, longitude: float) -> float:
    ...
    def _calculate_tajaka_balas(
        self, planets: Dict[str, Dict[str, Any]], ascendant: float
    ...
    # TAJAKA ASPECTS
    # =========================================================================
    def _calculate_tajaka_aspects(self, planets: Dict[str, Dict[str, Any]]) -> List[TajakaAspect]:
        ...
        for i, p1 in enumerate(planet_list):
            for p2 in planet_list[i + 1 :]:
                aspect = self._check_aspect(p1, planets[p1]["longitude"], p2, planets[p2]["longitude"])
                if aspect:
                    aspects.append(aspect)

        return aspects

    def _check_aspect(self, planet1: str, lon1: float, planet2: str, lon2: float) -> Optional[TajakaAspect]:
    ...
    def _get_aspect_type(self, angle: int, is_applying: bool) -> str:
        ...
        base_names = {0: "Conjunction", 60: "Sextile", 90: "Square", 120: "Trine", 180: "Opposition"}
    ...
    def _interpret_tajaka_aspect(self, faster: str, slower: str, aspect_type: str) -> str:
    ...
    def _calculate_tajaka_yogas(
        self, planets: Dict[str, Dict[str, Any]], aspects: List[TajakaAspect], ascendant: float
    ) -> List[TajakaYoga]:
        ...
        all_in_good = all(self._get_house(p["longitude"], ascendant) in [1, 4, 5, 7, 9, 10] for p in planets.values())
        yogas.append(
            TajakaYoga(
                name="Induvara Yoga",
        ...
        moon_ithasala = any(a.faster_planet == "Moon" or a.slower_planet == "Moon" for a in ithasala_aspects)
        yogas.append(
            TajakaYoga(
                name="Nakta Yoga",
        ...
        yogas.append(
            TajakaYoga(
                name="Yamaya Yoga",
                planets_involved=(
                    [ithasala_aspects[0].faster_planet, ithasala_aspects[0].slower_planet] if ithasala_aspects else []
                ),
    ...
    def _add_additional_tajaka_yogas(
        self, yogas: List[TajakaYoga], planets: Dict[str, Dict[str, Any]], aspects: List[TajakaAspect], ascendant: float
    ):
        """Add remaining Tajaka yogas"""
        # Manau Yoga - Significator strong
        jup_strong = (
            self._pancha_vargeeya_bala(
                "Jupiter",
        ...
        kamboola = (
            diff in range(0, 13)
            or diff in range(54, 66)
            or diff in range(84, 96)
            or diff in range(114, 126)
            or diff in range(174, 186)
        )
    ...
    def _generate_annual_prediction(
        self, planets: Dict[str, Dict[str, Any]], ascendant: float, muntha: int, yogas: List[TajakaYoga]
    ) -> Dict[str, str]:
    ...
    def _get_annual_advice(self, planets: Dict[str, Dict[str, Any]], ascendant: float, yogas: List[TajakaYoga]) -> str:
...
def calculate_tajaka_annual(birth_date: datetime, birth_sun_longitude: float, year_number: int) -> Dict[str, Any]:

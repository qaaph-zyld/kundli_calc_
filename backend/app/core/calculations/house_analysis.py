...
class EnhancedHouseAnalysisEngine:
    ...
    async def _calculate_occupant_strength(self, house: int, occupants: List[Dict[str, Any]]) -> float:
        ...
        for planet in occupants:
            futures.append(self.executor.submit(self._process_single_occupant, house, planet))

        # Gather results
        strengths = [f.result() for f in futures]
        return min(100, sum(strengths) / len(strengths))

    def _process_single_occupant(self, house: int, planet: Dict[str, Any]) -> float:
    ...
    def _calculate_aspect_strength(self, house: int, aspects: List[Dict[str, Any]]) -> float:
    ...
    def _calculate_lord_strength(self, house: int, lord: Dict[str, Any]) -> float:
    ...
    def _calculate_temporal_strength(self, house: int, time_of_day: str) -> float:
    ...
    def _calculate_dispositorship_strength(
        self, house: int, lord: Dict[str, Any], all_house_lords: Dict[int, Dict[str, Any]]
    ) -> float:
        ...
        for other_house, other_lord in all_house_lords.items():
            if other_house != house:
                if other_lord.get("planet") == lord_planet and other_lord.get("house") == house:
    ...
    async def analyze_house(
        ...
        dispositorship_strength = self._calculate_dispositorship_strength(house, lord, all_house_lords or {})
    ...
    def analyze_house_combinations(self, house_strengths: Dict[int, HouseStrength]) -> Dict[str, List[int]]:
        ...
        for combo_name, houses in self.special_combinations.items():
            ...
            combo_strength = sum(house_strengths[h].total_strength for h in houses) / len(houses)

            combinations[combo_name] = {"houses": list(houses), "strength": round(combo_strength, 2)}

        return combinations

    def get_strongest_houses(self, house_strengths: Dict[int, HouseStrength], top_n: int = 3) -> List[HouseStrength]:
        """Get the strongest houses by total strength"""
        sorted_houses = sorted(house_strengths.values(), key=lambda x: x.total_strength, reverse=True)
        return sorted_houses[:top_n]

    def get_weakest_houses(self, house_strengths: Dict[int, HouseStrength], bottom_n: int = 3) -> List[HouseStrength]:
        """Get the weakest houses by total strength"""
        sorted_houses = sorted(house_strengths.values(), key=lambda x: x.total_strength)

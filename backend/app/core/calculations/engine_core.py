...
class CoreCalculationEngine:
    ...
    async def execute_calculation(self, calc_type: str, params: Dict, validation_level: str = "strict") -> Dict:
        ...
        try:
            ...
            await self.validation_engine.validate_inputs(calc_type, params, validation_level)

            # Execute calculation with monitoring
            result = await self._perform_calculation(calc_type, params)

            # Post-calculation validation
            validation_result = await self.validation_engine.validate_result(calc_type, result, validation_level)

            # Collect metrics
            metrics = await self._collect_execution_metrics(start_time, result, validation_result)

            # Optimize based on metrics
            await self.optimization_engine.analyze_and_optimize(metrics)

            return {"result": result, "metrics": metrics, "validation": validation_result}

...
class KPSystem:
    ...
    def _get_sub_sub_lord(self, longitude: float, nakshatra: int, star_lord: str, sub_lord: str) -> str:
    ...
    def get_planet_significators(
        self, planet_name: str, planet_kp: KPPosition, planet_house: int, house_cusps_kp: Dict[int, KPPosition]
    ) -> KPSignificators:
        """
        ...
        return KPSignificators(planet_or_house=planet_name, A=A, B=B, C=C, D=D, combined=combined)
    ...
    def get_ruling_planets(
        self, current_time: datetime, moon_longitude: float, ascendant_longitude: float
    ) -> RulingPlanets:
        """
...
def get_kp_data(planets: Dict[str, float], house_cusps: List[float], current_time: datetime) -> Dict[str, Any]:
    """

...
class SudarshanaChakra:
    ...
    def calculate(
        ...
        for house in range(1, 13):
            ...
            houses[house] = {
                "house_number": house,
                "from_lagna": {
                    "sign": SIGNS[sign_from_lagna],
                    "sign_number": sign_from_lagna
                },
                "from_moon": {
                    "sign": SIGNS[sign_from_moon],
                    "sign_number": sign_from_moon
                },
                "from_sun": {
                    "sign": SIGNS[sign_from_sun],
                    "sign_number": sign_from_sun
                },
    ...
    def _analyze_sudarshana(
        ...
        for house in range(1, 13):
            h = houses[house]
            if (h["from_lagna"]["sign_number"] == h["from_moon"]["sign_number"] == 
                h["from_sun"]["sign_number"]):
                strong_houses.append(house)
        
        # Count planets in kendras from each
        kendras = [1, 4, 7, 10]
        kendra_strength = {
            "from_lagna": sum(1 for p, s in planet_signs.items() 
                            if ((s - asc_sign + 12) % 12) + 1 in kendras),
            "from_moon": sum(1 for p, s in planet_signs.items() 
                           if ((s - moon_sign + 12) % 12) + 1 in kendras),
            "from_sun": sum(1 for p, s in planet_signs.items() 
                          if ((s - sun_sign + 12) % 12) + 1 in kendras)
        }
...
class SarvatobhadraChakra:
    ...
    def _initialize_grid(self):
        ...
        nakshatra_positions = [
            (0, 4), (0, 5), (0, 6), (0, 7), (0, 8),  # Top
            (1, 8), (2, 8), (3, 8), (4, 8),           # Right
            (5, 8), (6, 8), (7, 8), (8, 8),           # Right continued
            (8, 7), (8, 6), (8, 5), (8, 4),           # Bottom
            (8, 3), (8, 2), (8, 1), (8, 0),           # Bottom continued
            (7, 0), (6, 0), (5, 0), (4, 0),           # Left
            (3, 0), (2, 0), (1, 0)                     # Left continued
        ]
...
def calculate_all_chakras(
    ...
    return {
        ...
        "sarvatobhadra_chakra": sarvatobhadra_data,
        "kota_chakra": kota_data
    }

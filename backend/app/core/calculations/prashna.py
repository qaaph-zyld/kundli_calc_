...
class PrashnaCalculator:
    ...
    def analyze_prashna(
        ...
        relevant_houses = self.question_house_map.get(
            question_category, [1, 7, 10]
        )
        
        # Analyze each relevant house
        house_analysis = {}
        for house in relevant_houses:
            house_analysis[house] = self._analyze_house(
                house, asc_sign, planets, ascendant
            )
        
        # Check overall favorability
        favorable = self._check_favorability(
            asc_sign, moon_sign, planets, ascendant, relevant_houses
        )
        ...
        return {
            ...
            "arudha": {
                "sign": SIGNS[arudha],
                "interpretation": self._interpret_arudha(arudha, question_category)
            },
    ...
    def _calculate_arudha(
        ...
        lord_signs = {
            "Mars": 0, "Venus": 1, "Mercury": 2, "Moon": 3,
            "Sun": 4, "Jupiter": 8, "Saturn": 9
        }
    ...
    def _analyze_house(
        ...
        strength = self._calculate_house_strength(
            house, house_sign, planets_in_house, lord_house
        )
    ...
    def _analyze_lagna(
        ...
        # Planets in Lagna
        planets_in_lagna = [
            p for p, lon in planets.items()
            if int(lon / 30) == asc_sign
        ]
    ...
    def _analyze_moon(
        ...
        return {
            ...
            "interpretation": "Void of Course - delay likely" if void_of_course 
                            else "Moon active - matter will progress"
        }
    ...
    def _check_shakuna(
        ...
        return {
            "omens": omens,
            "overall": "Positive omens" if len([o for o in omens if "growth" in o["omen"].lower()]) > 0 else "Mixed omens"
        }
    
    def _interpret_arudha(
        self,
        arudha: int,
        category: QuestionCategory
    ) -> str:

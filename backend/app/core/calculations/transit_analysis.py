...
class TransitAnalyzer:
    ...
    def analyze_transit(self, transit_positions: Dict[str, float], current_time: datetime) -> Dict[str, Any]:
        """
    ...
    def _analyze_gochara(self, planet: str, longitude: float, all_transits: Dict[str, float]) -> GocharaResult:
        """
        ...
        interpretation = self._generate_interpretation(planet, house_from_moon, is_benefic, has_vedha, vedha_planet)
    ...
    def _generate_interpretation(
        self, planet: str, house: int, is_benefic: bool, has_vedha: bool, vedha_planet: Optional[str]
    ) -> str:
    ...
    def _get_transit_effects(self, planet: str, house: int, is_benefic: bool) -> List[str]:
    ...
    def _calculate_overall_score(self, gochara_results: List[GocharaResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": status,
            "favorable_planets": [
                r.planet for r in gochara_results if r.overall_result in [TransitResult.EXCELLENT, TransitResult.GOOD]
            ],
            "challenging_planets": [
                r.planet
                for r in gochara_results
                if r.overall_result in [TransitResult.CHALLENGING, TransitResult.DIFFICULT]
            ],
        }

    def _generate_predictions(
        self, gochara_results: List[GocharaResult], sade_sati: SadeSatiStatus
    ) -> List[Dict[str, str]]:
    ...
    def _identify_important_transits(self, positions: List[TransitPosition]) -> List[Dict[str, str]]:
...
def analyze_transits(
    natal_moon_sign: int, natal_planets: Dict[str, float], current_positions: Dict[str, float], current_time: datetime
) -> Dict[str, Any]:
    """
...
def get_current_transits_summary(
    natal_moon_sign: int, current_saturn: float, current_jupiter: float, current_rahu: float
) -> Dict[str, str]:
    """
    ...
    # Overall assessment
    favorable_count = sum(
        [summary["saturn"]["is_favorable"], summary["jupiter"]["is_favorable"], summary["rahu"]["is_favorable"]]
    )
...
def get_current_transit_positions(
    target_datetime: datetime = None, ayanamsa_type: str = "lahiri"
    ...
    for planet_name, planet_id in planets.items():
        try:
            ...
            positions[planet_name] = {"error": str(e), "longitude": 0, "sign": "Unknown", "is_retrograde": False}
...
def get_transit_to_natal_aspects(
    transit_positions: Dict[str, float], natal_positions: Dict[str, float], orb: float = 10.0
) -> List[Dict[str, Any]]:
    """
    ...
    special_aspects = {"Mars": [90, 210], "Saturn": [60, 270], "Jupiter": [120, 240]}
...
def get_upcoming_transits(natal_moon_sign: int, days_ahead: int = 30) -> List[Dict[str, Any]]:
    """

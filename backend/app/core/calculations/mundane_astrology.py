...
class MundaneCalculator:
    ...
    def calculate_solar_ingress(
        ...
        return {
            ...
            "ascendant": {"longitude": ascendant, "sign": SIGNS[int(ascendant / 30)]},
            "planets": {p: {"longitude": lon, "sign": SIGNS[int(lon / 30)]} for p, lon in planets.items()},
            "analysis": analysis,
            "validity": "1 year" if ingress_type == "aries" else "6 months",
        }

    def _analyze_ingress_chart(self, planets: Dict[str, float], ascendant: float, ingress_type: str) -> Dict[str, Any]:
        """Analyze ingress chart for mundane predictions"""
        lagna = int(ascendant / 30)

        # House positions
        houses = {planet: ((int(lon / 30) - lagna) % 12) + 1 for planet, lon in planets.items()}
    ...
    def _interpret_lunar_chart(self, planets: Dict[str, float], ascendant: float, chart_type: str) -> Dict[str, str]:
    ...
    # =========================================================================
    def compress_dasha(self, dasha_periods: List[Dict[str, Any]], compress_to_years: float) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_ascendant(self, time: datetime, latitude: float, longitude: float) -> float:
...
def calculate_mundane_chart(
    chart_type: str, date: datetime, latitude: float = 28.6139, longitude: float = 77.2090
) -> Dict[str, Any]:

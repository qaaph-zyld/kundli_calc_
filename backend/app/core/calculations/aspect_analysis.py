...
class AspectAnalyzer:
    ...
    def calculate_aspect_influence(
        self, aspecting_planet: str, aspected_planet: str, houses_between: int
        ...
        relationship_factor = self._calculate_relationship_factor(aspecting_planet, aspected_planet)

        # Calculate final strength
        final_strength = base_strength * relationship_factor

        # Determine if aspect is beneficial
        is_beneficial = self._is_aspect_beneficial(aspecting_planet, aspected_planet, houses_between)

        # Check for special effects
        special_effects = self._check_special_effects(aspecting_planet, aspected_planet, houses_between)
        ...
        for i, p1 in enumerate(planets):
            for p2 in planets[i + 1 :]:
                ...
                reverse_houses = self._calculate_houses_between(
                    planet_positions[p2]["longitude"], planet_positions[p1]["longitude"]
                )
    ...
    def _is_aspect_beneficial(self, aspecting_planet: str, aspected_planet: str, houses: int) -> bool:
    ...
    def _check_special_effects(self, aspecting_planet: str, aspected_planet: str, houses: int) -> List[str]:
    ...
    def _calculate_houses_between(self, longitude1: float, longitude2: float) -> int:

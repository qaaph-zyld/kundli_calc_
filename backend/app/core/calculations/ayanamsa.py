...
from app.core.validation.ayanamsa_validator import AyanamsaValidationError, AyanamsaValidator
...
class EnhancedAyanamsaManager:
    ...
    def __init__(self):
        ...
        self.ayanamsa_systems = {
            "LAHIRI": {"id": "lahiri", "historical_correction": 0.0, "annual_precession": 50.27},  # arcseconds per year
            "RAMAN": {"id": "raman", "historical_correction": 0.15, "annual_precession": 50.27},
            "KRISHNAMURTI": {"id": "krishnamurti", "historical_correction": 0.25, "annual_precession": 50.27},
            "YUKTESHWAR": {"id": "yukteshwar", "historical_correction": -0.12, "annual_precession": 50.27},
            "JN_BHASIN": {"id": "jn_bhasin", "historical_correction": 0.18, "annual_precession": 50.27},
        }
    ...
    def compare_systems(self, date: datetime) -> Dict[str, float]:
        ...
        return {system: self.calculate_precise_ayanamsa(date, system) for system in self.ayanamsa_systems}
...
class AyanamsaSystem:
    ...
    @classmethod
    def calculate_ayanamsa(cls, date: datetime, system: str = "lahiri") -> float:
        """
    ...
    @classmethod
    def apply_ayanamsa(cls, tropical_position: float, ayanamsa: float) -> float:
        """
    ...
    @classmethod
    def convert_all_positions(
        cls, positions: Dict[str, float], date: datetime, system: str = "lahiri"
    ) -> Dict[str, float]:
        """
        ...
        return {planet: cls.apply_ayanamsa(pos, ayanamsa) for planet, pos in positions.items()}
...
class MockAyanamsaManager:
    ...
    def get_ayanamsa(self, date: datetime, ayanamsa_type: str = "lahiri") -> float:
    ...
    def apply_ayanamsa(self, longitude: float, date: datetime, ayanamsa_type: str = "lahiri") -> float:
    ...
    def convert_to_tropical(self, sidereal_longitude: float, date: datetime, ayanamsa_type: str = "lahiri") -> float:

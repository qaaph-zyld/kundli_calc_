...
class TransitSearcher:
    ...
    def search_planetary_return(
        self, planet: str, natal_longitude: float, current_longitude: float, start_date: datetime
    ) -> Optional[TransitEvent]:
        """
    ...
    def search_sign_ingress(
        self, transit_planet: str, target_sign: int, current_transit_lon: float, start_date: datetime
    ) -> Optional[TransitEvent]:
        """
    ...
    def search_specific_transit(
        ...
        if target_type == "degree":
            return self.search_conjunction(transit_planet, float(target_value), transit_current_lon, start_date)

        elif target_type == "sign":
            return self.search_sign_ingress(transit_planet, int(target_value), transit_current_lon, start_date)

        elif target_type == "house":
            house_num = int(target_value)
            house_cusp = (natal_ascendant + (house_num - 1) * 30) % 360
            return self.search_house_ingress(transit_planet, house_cusp, transit_current_lon, start_date, house_num)
...
def search_transits(
    ...
    if search_type == "major":
        return searcher.search_major_transits(natal_planets, transit_planets, start_date, days_ahead)
    ...
    for transit_planet, transit_lon in transit_planets.items():
        for natal_planet, natal_lon in natal_planets.items():
            events = searcher.search_all_aspects_to_planet(
                transit_planet, natal_planet, natal_lon, transit_lon, start_date, days_ahead
            )
            all_events.extend(events)

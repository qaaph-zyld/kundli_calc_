...
class PanchangCalculator:
    ...
    def _calculate_tithi(self, sun_lon: float, moon_lon: float) -> Dict:
        ...
        return {"number": tithi_num, "name": tithi_name, "paksha": paksha, "remaining_degrees": 12 - (diff % 12)}
    ...
    def _calculate_yoga(self, sun_lon: float, moon_lon: float) -> Dict:
        ...
        return {"name": YOGA_NAMES[yoga_idx], "quality": YOGA_QUALITY[yoga_idx], "index": yoga_idx}
    ...
    def _calculate_sun_times(self, date: datetime, lat: float, lon: float) -> Tuple[datetime, datetime]:
        ...
        sunrise = date.replace(hour=int(sunrise_hour), minute=int((sunrise_hour % 1) * 60), second=0)
        sunset = date.replace(hour=int(sunset_hour), minute=int((sunset_hour % 1) * 60), second=0)
...
class MuhurtaCalculator:
    ...
    def get_rahu_kalam(self, date: datetime, sunrise: datetime, sunset: datetime) -> Tuple[datetime, datetime]:
        """
    ...
    def get_yamagandam(self, date: datetime, sunrise: datetime, sunset: datetime) -> Tuple[datetime, datetime]:
    ...
    def get_gulika_kalam(self, date: datetime, sunrise: datetime, sunset: datetime) -> Tuple[datetime, datetime]:
    ...
    def get_abhijit_muhurta(self, sunrise: datetime, sunset: datetime) -> Tuple[datetime, datetime]:
        """
    ...
    def get_brahma_muhurta(self, sunrise: datetime) -> Tuple[datetime, datetime]:
        """
    ...
    def get_choghadiya(self, date: datetime, sunrise: datetime, sunset: datetime) -> List[Dict]:
        """
    ...
    def get_hora(self, date_time: datetime, sunrise: datetime) -> Dict:
        """
    ...
    def find_good_muhurtas(self, date: datetime, activity: str, panchang: PanchangData) -> List[MuhurtaWindow]:
        """
        ...
        abhijit_start, abhijit_end = self.get_abhijit_muhurta(panchang.sunrise, panchang.sunset)
        muhurtas.append(
            MuhurtaWindow(
        ...
        choghadiyas = self.get_choghadiya(date, panchang.sunrise, panchang.sunset)
    ...
    def get_inauspicious_times(self, date: datetime, sunrise: datetime, sunset: datetime) -> Dict:
...
def get_daily_panchang(
    date_time: datetime, sun_lon: float, moon_lon: float, latitude: float = 28.6139, longitude: float = 77.2090
) -> Dict[str, Any]:
    """
    ...
    panchang = panchang_calc.calculate_panchang(date_time, sun_lon, moon_lon, latitude, longitude)

    inauspicious = muhurta_calc.get_inauspicious_times(date_time, panchang.sunrise, panchang.sunset)

    choghadiyas = muhurta_calc.get_choghadiya(date_time, panchang.sunrise, panchang.sunset)
    ...
    return {
        ...
        "tithi": {"name": panchang.tithi, "number": panchang.tithi_number, "paksha": panchang.tithi_paksha},
        "nakshatra": {"name": panchang.nakshatra, "lord": panchang.nakshatra_lord, "pada": panchang.nakshatra_pada},
        "yoga": {"name": panchang.yoga, "quality": panchang.yoga_quality},

...
class PlanetaryStrengthCalculator:
    ...
    def calculate_strength(
        self, planet: str, longitude: float, chart_time: datetime, house_position: int
    ) -> PlanetaryStrength:
        ...
        shadbala = self._calculate_shadbala(dignity, positional, temporal, aspect)

        # Calculate total strength
        total = self._calculate_total_strength([dignity, positional, temporal, aspect, shadbala])
    ...
    def _calculate_shadbala(self, dignity: float, positional: float, temporal: float, aspect: float) -> float:
        """Calculate six-fold strength (Shadbala)"""
        # Implement Shadbala calculation
        # Consider all six strength factors
        weights = {"dignity": 0.3, "positional": 0.2, "temporal": 0.2, "aspect": 0.3}

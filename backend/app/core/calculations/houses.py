...
from app.core.validation.house_validator import HouseValidationError, HouseValidator
...
class HouseCalculator:
    ...
    def __init__(self):
        ...
        self.house_systems = {
            "PLACIDUS": {"id": "P", "description": "Placidus house system", "time_based": True},
            "KOCH": {"id": "K", "description": "Koch house system", "time_based": True},
            "EQUAL": {"id": "E", "description": "Equal house system", "time_based": False},
            "WHOLE_SIGN": {"id": "W", "description": "Whole sign houses", "time_based": False},
            "REGIOMONTANUS": {"id": "R", "description": "Regiomontanus house system", "time_based": True},
            "CAMPANUS": {"id": "C", "description": "Campanus house system", "time_based": True},
        }

    def _normalize_house_system(self, house_system: Optional[str]) -> str:
        if house_system is None:
            house_system = self.house_system

        hs = str(house_system).strip()
        hs_up = hs.upper()
        ...
        if hs_up in code_map:
            return code_map[hs_up]

        hs_up = hs_up.replace("-", "_").replace(" ", "_")
        if hs_up == "WHOLESIGN":
            return "WHOLE_SIGN"
    ...
    def calculate_houses(
        self, date: datetime, latitude: float, longitude: float, house_system: str = None
    ) -> Dict[str, List[float]]:
        ...
        """
        house_system = self._normalize_house_system(house_system)
        ...
        return {"cusps": house_cusps, "ascendant": ascendant, "midheaven": midheaven, "armc": armc, "vertex": vertex}
    ...
    def get_house_system_info(self) -> Dict[str, Dict[str, str]]:
        ...
        """
        return {
            name: {"id": system["id"], "description": system["description"]}
            for name, system in self.house_systems.items()
        }

    def get_house_for_longitude(self, longitude: float, house_cusps: List[float]) -> int:
    ...
    def calculate_house_positions(self, planet_positions: Dict[str, float], house_cusps: List[float]) -> Dict[str, int]:
        ...
        """
        return {planet: self.get_house_for_longitude(pos, house_cusps) for planet, pos in planet_positions.items()}

    def calculate_interceptions(self, house_cusps: List[float]) -> List[Tuple[int, float, float]]:

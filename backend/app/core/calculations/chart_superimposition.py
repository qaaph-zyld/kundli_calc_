...
class ChartSuperimposition:
    ...
    def superimpose_charts(
        ...
        if chart1_ascendant is not None:
            house_overlays = self._analyze_house_overlays(
                chart1, chart2, chart1_ascendant, chart2_name
            )
    ...
    def _interpret_aspect(self, aspect: SuperimposedAspect) -> str:
        ...
        base = interpretations.get(key) or interpretations.get(rev_key) or f"{aspect.planet1}-{aspect.planet2} interaction"
    ...
    def _generate_summary(
        self,
        aspects: List[SuperimposedAspect],
    ...
    def natal_transit_overlay(
        ...
        result = self.superimpose_charts(
            natal_planets, "Natal",
        ...
        result["transit_analysis"] = self._analyze_transits(
            natal_planets, transit_planets, natal_ascendant
        )
    ...
    def divisional_overlay(
        ...
        result = self.superimpose_charts(
            rasi_planets, "Rasi (D-1)",
            divisional_planets, divisional_name,
            ascendant
        )
...
def superimpose_charts(
    chart1: Dict[str, float],
    chart2: Dict[str, float],
    chart1_name: str = "Chart 1",
    chart2_name: str = "Chart 2"
) -> Dict[str, Any]:

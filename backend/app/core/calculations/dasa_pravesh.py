...
class DasaPraveshCalculator:
    ...
    def calculate_pravesh_chart(
        ...
        transit_planets = self._calculate_transit_positions(
            natal_planets, 
            dasa_start_time
        )
        
        # Calculate ascendant for dasa start time
        pravesh_ascendant = self._calculate_pravesh_ascendant(
            dasa_start_time,
            birth_latitude,
            birth_longitude
        )
    ...
    def _calculate_pravesh_ascendant(
        self,
        time: datetime,
        latitude: float,
        longitude: float
    ...
    def _analyze_pravesh_chart(
        ...
        pravesh_houses = {
            planet: ((int(lon/30) - pravesh_lagna) % 12) + 1
    ...
    def _get_house_relation(self, sign1: int, sign2: int) -> str:
        ...
        diff = (sign1 - sign2) % 12
        relations = {
            0: "Same (1st)", 1: "2nd", 2: "3rd", 3: "4th (Kendra)",
            4: "5th (Trikona)", 5: "6th (Dusthana)", 6: "7th (Kendra)",
            7: "8th (Dusthana)", 8: "9th (Trikona)", 9: "10th (Kendra)",
            10: "11th", 11: "12th (Dusthana)"
        }
    ...
    def _analyze_malefic_positions(self, houses: Dict[str, int]) -> Dict[str, Any]:
        ...
        for planet in malefics:
            house = houses.get(planet, 0)
            malefic_analysis[planet] = {
                "house": house,
                "favorable": house in upachaya
            }
        
        return {
            "details": malefic_analysis,
            "assessment": "Malefics well-placed" if sum(1 for m in malefic_analysis.values() if m["favorable"]) >= 3 else "Some malefic challenges"
        }
    
    def _analyze_key_transits(
        self,
...
def calculate_dasa_pravesh(
    ...
    chart = calc.calculate_pravesh_chart(
        natal_planets, natal_ascendant, dasa_start, dasa_lord, level
    )
    ...
    return {
        ...
        "ascendant": {
            ...
            "sign": SIGNS[int(chart.ascendant / 30)]
        },
        "planets": {

...
class HorizontalScaling:
    ...
    def __init__(self, min_replicas: int = 1, max_replicas: int = 10, cooldown_seconds: int = 300):
        """Initialize strategy"""
    ...
    async def scale(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if target > self.metrics.current_capacity:
                await self._scale_up(target - self.metrics.current_capacity)
            elif target < self.metrics.current_capacity:
                await self._scale_down(self.metrics.current_capacity - target)
    ...
    def _calculate_target_capacity(self, config: Dict[str, Any]) -> int:
        """Calculate target capacity"""
        # Get current metrics
...
class VerticalScaling:
    ...
    def __init__(self, min_resources: Dict[str, float], max_resources: Dict[str, float], cooldown_seconds: int = 300):
        """Initialize strategy"""
        ...
        try:
            ...
            for resource, value in target.items():
                if value > current.get(resource, 0.0):
                    await self._scale_up_resource(resource, value)
                elif value < current.get(resource, 0.0):
                    await self._scale_down_resource(resource, value)
    ...
    def _calculate_target_resources(self, config: Dict[str, Any]) -> Dict[str, float]:
        """Calculate target resources"""
        target = {}

        # Calculate CPU target
        cpu_usage = config.get("cpu_usage", 0.0)
        current_cpu = self.metrics.resource_utilization.get("cpu", self.min_resources["cpu"])

        if cpu_usage > 0.8:
            target["cpu"] = min(current_cpu * 1.5, self.max_resources["cpu"])
        elif cpu_usage < 0.2:
            target["cpu"] = max(current_cpu * 0.75, self.min_resources["cpu"])
        else:
            target["cpu"] = current_cpu

        # Calculate memory target
        memory_usage = config.get("memory_usage", 0.0)
        current_memory = self.metrics.resource_utilization.get("memory", self.min_resources["memory"])

        if memory_usage > 0.8:
            target["memory"] = min(current_memory * 1.5, self.max_resources["memory"])
        elif memory_usage < 0.2:
            target["memory"] = max(current_memory * 0.75, self.min_resources["memory"])
        else:
            target["memory"] = current_memory

        return target

    async def _scale_up_resource(self, resource: str, target: float):
        """Scale up resource"""
        self.state = ScalingState.SCALING_UP
        self.metrics.scale_up_count += 1
    ...
    async def _scale_down_resource(self, resource: str, target: float):
        """Scale down resource"""
        self.state = ScalingState.SCALING_DOWN
        self.metrics.scale_down_count += 1
...
class HybridScaling:
    ...
    def __init__(self, horizontal: HorizontalScaling, vertical: VerticalScaling):
        """Initialize strategy"""
    ...
    async def scale(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not vertical_result:
                horizontal_result = await self.horizontal.scale(config)

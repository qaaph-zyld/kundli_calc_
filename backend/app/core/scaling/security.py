...
class ScalingSecurity:
    ...
    def __init__(self, config_path: Optional[str] = None, mode: SecurityMode = SecurityMode.STANDARD):
        """Initialize security"""
        self.mode = mode
    ...
    def _init_logger(self):
        ...
        if not self.logger.handlers:
            ...
            # File handler
            file_handler = logging.FileHandler("scaling_security.log")
    ...
    def create_token(self, user: str, scopes: List[SecurityScope]) -> str:
        """Create JWT token"""
        try:
            # Create payload
            payload = {
            ...
            # Create token
            token = jwt.encode(payload, self.config.jwt_secret.get_secret_value(), algorithm=self.config.jwt_algorithm)
    ...
    def validate_token(self, credentials: HTTPAuthorizationCredentials = Security(HTTPBearer())) -> Dict[str, Any]:
        """Validate JWT token"""
        try:
            # Decode token
            payload = jwt.decode(
    ...
    def validate_scope(self, token_payload: Dict[str, Any], required_scope: SecurityScope):
        """Validate security scope"""
        if required_scope.value not in token_payload.get("scopes", []):
            raise HTTPException(status_code=403, detail="Insufficient permissions")

    def validate_rate_limit(self, user: str, action: SecurityAction):
        """Validate rate limit"""
        # Get recent actions
        recent_actions = [
            a
            ...
            if (
                a.user == user
                and a.action == action
                and a.timestamp > datetime.utcnow() - timedelta(seconds=self.config.rate_window)
            )
        ]
    ...
    def audit_action(self, action: SecurityAction, user: str, status: str, details: Dict[str, Any]):
        """Audit security action"""
        try:
            # Create audit entry
            audit = SecurityAudit(action=action, timestamp=datetime.utcnow(), user=user, status=status, details=details)

            # Add to audit log
            self.audit_log.append(audit)

            # Log action
            self.logger.info(f"Security audit: {action.value} by {user}")
    ...
    def get_audit_log(
        ...
        # Filter by user
        if user:
            filtered_log = [a for a in filtered_log if a.user == user]

...
# Module-level OAuth2 scheme for dependency injection
...
class SecurityEngine:
    ...
    async def get_current_user(self, token: str = Depends(oauth2_scheme)) -> User:
        ...
        try:
            ...
            token_data = TokenData(
                username=username, scopes=payload.get("scopes", []), exp=datetime.fromtimestamp(payload.get("exp"))
            )
        except JWTError:
            raise credentials_exception
    ...
    async def verify_scope(self, user: User, required_scope: SecurityScope) -> bool:
        ...
        return any(
            scope.name == required_scope.name and all(perm in scope.permissions for perm in required_scope.permissions)
    ...
    async def get_user_scopes(self, user: User) -> List[SecurityScope]:
        ...
        await self.redis_cache.set(cache_key, [scope.dict() for scope in scopes], expire=settings.CACHE_TTL_SECONDS)
        return scopes

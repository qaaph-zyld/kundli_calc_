...
class AuthorizationManager:
    ...
    def get_policies_by_role(self, role: Role) -> List[Policy]:
        ...
        return [policy for policy in self.policies.values() if policy.role == role]

    def has_permission(
        self, role: Role, resource_type: ResourceType, permission: Permission, context: Optional[Dict[str, Any]] = None
    ) -> bool:
        """Check if role has permission for resource"""
        policies = self.get_policies_by_role(role)

        for policy in policies:
            if policy.resource_type == resource_type and permission in policy.permissions:
    ...
    def get_user_permissions(self, role: Role, resource_type: ResourceType) -> Set[Permission]:

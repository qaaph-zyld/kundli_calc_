...
from .registry import ServiceDefinition, ServiceEndpoint, ServiceType
...
class ConfigManager:
    ...
    def _initialize_sources(self) -> None:
        ...
        self.add_source(
            ConfigurationSource(name="local", type="file", location=os.path.join(self._config_dir, "local"), priority=1)
        )

        self.add_source(ConfigurationSource(name="environment", type="env", location="", priority=2))
    ...
    def _load_from_source(self, source: ConfigurationSource, service_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _load_from_file(self, source: ConfigurationSource, service_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _load_from_remote(self, source: ConfigurationSource, service_name: str) -> Optional[Dict[str, Any]]:
    ...
    def update_config(self, service_name: str, updates: Dict[str, Any]) -> ServiceConfig:
    ...
    def create_service_definition(self, service_name: str) -> ServiceDefinition:
        """Create service definition from configuration"""
        config = self.get_config(service_name)

        # Convert config endpoints to ServiceEndpoint objects
        endpoints = {name: ServiceEndpoint(**endpoint_config) for name, endpoint_config in config.endpoints.items()}

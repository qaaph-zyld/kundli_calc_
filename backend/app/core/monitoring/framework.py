...
class MetricsManager:
    ...
    def create_metric(self, config: MetricConfig) -> None:
        ...
        if config.type == MetricType.COUNTER:
            metric = Counter(config.name, config.description, config.labels)
        elif config.type == MetricType.GAUGE:
            metric = Gauge(config.name, config.description, config.labels)
        elif config.type == MetricType.HISTOGRAM:
            metric = Histogram(
                config.name, config.description, config.labels, buckets=config.buckets or Histogram.DEFAULT_BUCKETS
            )
        else:  # SUMMARY
            metric = Summary(
                config.name, config.description, config.labels, quantiles=config.quantiles or [0.5, 0.9, 0.99]
            )
...
class AlertManager:
    ...
    def check_alert(self, rule: AlertRule, value: float, current_time: datetime) -> Optional[Alert]:
...
class SystemMetricsCollector:
    ...
    def _setup_metrics(self) -> None:
        ...
        metrics = {
            "system_cpu_usage": MetricConfig(
                name="system_cpu_usage", type=MetricType.GAUGE, description="System CPU usage percentage"
            ),
            "system_memory_usage": MetricConfig(
                name="system_memory_usage", type=MetricType.GAUGE, description="System memory usage percentage"
            ),
    ...
    async def collect_metrics(self) -> None:
        ...
        # CPU usage
        self.metrics.set_gauge("system_cpu_usage", psutil.cpu_percent())
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                self.metrics.set_gauge("system_disk_usage", usage.percent, {"path": partition.mountpoint})
            except Exception:
                continue

        # Network IO
        net_io = psutil.net_io_counters()
        self.metrics.increment_counter("system_network_io", net_io.bytes_sent, {"direction": "sent"})
        self.metrics.increment_counter("system_network_io", net_io.bytes_recv, {"direction": "received"})
...
class MonitoringManager:
    ...
    async def start(self) -> None:
        ...
        await asyncio.gather(self._collect_metrics(), self._check_alerts())
            ...
            try:
                ...
                for rule in self.config.alerts.values():
                    ...
                    if metric:
                        ...
                        # Check alert rule
                        alert = self.alerts.check_alert(rule, value, current_time)
    ...
    def record_request(
        self, endpoint: str, method: str, duration: float, status: int, error: Optional[str] = None
    ) -> None:
        ...
        if error:
            self.metrics.increment_counter("http_request_errors_total", 1, {**labels, "error": error})

    def record_calculation(self, type: str, duration: float, success: bool) -> None:
        """Record calculation metrics"""
        labels = {"type": type, "success": str(success).lower()}

        # Increment calculation counter
        self.metrics.increment_counter("calculations_total", 1, labels)

        # Record calculation duration
        self.metrics.observe_histogram("calculation_duration_seconds", duration, labels)
    ...
    def record_cache(self, operation: str, hit: bool) -> None:
        ...
        self.metrics.increment_counter("cache_operations_total", 1, labels)

        # Update cache hit ratio
        if operation == "get":
            self.metrics.increment_counter("cache_hits_total" if hit else "cache_misses_total", 1)

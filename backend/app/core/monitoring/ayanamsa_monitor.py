...
class AyanamsaMonitor:
    ...
    def track_calculation(self):
        ...
        try:
            ...
            self.logger.debug(f"Calculation completed in {metrics.duration:.3f}s " f"(success={metrics.success})")

    def get_metrics(self) -> Dict[str, Any]:
        """Get current monitoring metrics"""
        avg_duration = self.total_duration / self.total_calculations if self.total_calculations > 0 else 0.0

        success_rate = (
            self.successful_calculations / self.total_calculations * 100 if self.total_calculations > 0 else 0.0
        )

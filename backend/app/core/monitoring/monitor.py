...
class MonitoringSystem:
    ...
    async def track_metric(
        self, name: str, value: float, metric_type: str = MetricType.GAUGE, labels: Optional[Dict[str, str]] = None
    ):
        """Track a metric"""
        metric = Metric(name=name, type=metric_type, value=value, labels=labels or {})
    ...
    async def track_endpoint(self, path: str, method: str, status_code: int, response_time: float):
        """Track endpoint metrics"""
        metric = EndpointMetrics(path=path, method=method, status_code=status_code, response_time=response_time)
        ...
        for endpoint, metrics in self.endpoint_metrics.items():
            ...
            if filtered_metrics:
                summary["endpoints"][endpoint] = {
                    ...
                    "p95_response_time": self._calculate_percentile([m.response_time for m in filtered_metrics], 95),
                }

        # Get performance metrics
        filtered_perf_metrics = [m for m in self.performance_metrics if start_time <= m.timestamp <= end_time]

...
class ErrorRecoverySystem:
    ...
    async def create_checkpoint(self, calculation_id: str, state: CalculationState) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Checkpoint created for calculation {calculation_id}")
            return True

        except Exception as e:
            self.logger.error(f"Failed to create checkpoint for {calculation_id}: {str(e)}")
            return False

    async def restore_checkpoint(self, calculation_id: str) -> Optional[CalculationState]:
        """Restore calculation state from checkpoint"""
        try:
            checkpoint_path = self._get_checkpoint_path(calculation_id)

            if not checkpoint_path.exists():
                self.logger.warning(f"No checkpoint found for calculation {calculation_id}")
                return None
            ...
            self.logger.info(f"Checkpoint restored for calculation {calculation_id}")
            return state

        except Exception as e:
            self.logger.error(f"Failed to restore checkpoint for {calculation_id}: {str(e)}")
            return None
    ...
    async def cleanup_checkpoints(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            for checkpoint in self.checkpoint_dir.glob("*.checkpoint"):
                if (
                    current_time - datetime.fromtimestamp(checkpoint.stat().st_mtime)
...
class TransactionLog:
    ...
    async def log_transaction(self, calculation_id: str, action: str, details: Dict[str, Any]) -> bool:
    ...
    async def get_transaction_history(self, calculation_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve transaction history: {str(e)}")
            return []
...
class RecoveryManager:
    ...
    async def start_calculation(self, calculation_id: str, input_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Log transaction
            await self.transaction_log.log_transaction(calculation_id, "start", {"input_data": input_data})
    ...
    async def update_calculation_state(
        self, calculation_id: str, phase: str, results: Dict[str, Any], completed_step: str
    ) -> bool:
        ...
        try:
            ...
            await self.transaction_log.log_transaction(
                calculation_id, "update", {"phase": phase, "completed_step": completed_step, "results": results}
            )
    ...
    async def complete_calculation(self, calculation_id: str, final_results: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Log transaction
            await self.transaction_log.log_transaction(calculation_id, "complete", {"final_results": final_results})
    ...
    async def handle_error(self, calculation_id: str, error: Exception) -> Optional[CalculationState]:
        """Handle calculation error and attempt recovery"""
        try:
            # Log error
            await self.transaction_log.log_transaction(calculation_id, "error", {"error": str(error)})
            ...
            await self.transaction_log.log_transaction(
                calculation_id, "recovery_attempt", {"recovered_state": state.__dict__}
            )

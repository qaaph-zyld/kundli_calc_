...
class YogaActivationEngine:
    ...
    def calculate_activation_windows(
        ...
        primary_periods = self._calculate_primary_periods(involved_planets, formation_strength)

        # Calculate peak activation (antardashas)
        peak_activation = self._calculate_peak_antardashas(involved_planets, formation_strength)

        # Identify transit triggers
        transit_triggers = self._identify_transit_triggers(yoga_name, involved_planets)

        # Generate manifestation timeline
        timeline = self._generate_manifestation_timeline(primary_periods, formation_strength)

        # Generate overall timing note
        timing_note = self._generate_timing_note(yoga_name, primary_periods, formation_strength)

        # Calculate confidence
        confidence = self._calculate_confidence(len(involved_planets), formation_strength)
    ...
    def _calculate_primary_periods(self, planets: List[str], formation_strength: float) -> List[DashaPeriod]:
        ...
        for planet in planets:
            ...
            periods.append(DashaPeriod(planet=planet, duration_years=duration, activation_strength=activation_strength))
    ...
    def _calculate_peak_antardashas(self, planets: List[str], formation_strength: float) -> List[Dict[str, Any]]:
    ...
    def _identify_transit_triggers(self, yoga_name: str, planets: List[str]) -> List[str]:
        ...
        if any(p in ["Jupiter", "Venus", "Mercury"] for p in planets):
            triggers.append("Jupiter transit through natal positions of involved planets")

        # Saturn transits activate karmic yogas
        if "Saturn" in planets or "Raja" in yoga_name:
            triggers.append("Saturn transit through 10th house or natal Saturn")

        # Rahu/Ketu transits for unconventional yogas
        if "Rahu" in planets or "Ketu" in planets:
            triggers.append("Rahu/Ketu transit through involved houses")

        # Generic trigger
        if not triggers:
            triggers.append("Jupiter or Saturn transit through houses containing yoga planets")

        return triggers

    def _generate_manifestation_timeline(self, periods: List[DashaPeriod], formation_strength: float) -> Dict[str, str]:
        """Generate manifestation timeline"""
    ...
    def _generate_timing_note(self, yoga_name: str, periods: List[DashaPeriod], formation_strength: float) -> str:
        """Generate overall timing note"""
    ...
    def _calculate_confidence(self, planet_count: int, formation_strength: float) -> float:
    ...
    def analyze_yoga_timing_batch(
        self, active_yogas: List[str], chart_data: Dict[str, Any]
    ...
    def _extract_involved_planets(self, yoga_name: str, yoga_data: Dict[str, Any]) -> List[str]:

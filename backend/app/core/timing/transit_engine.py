...
class TransitIntelligenceEngine:
    ...
    def get_current_transits(
        self, birth_data: Dict[str, Any], current_date: Optional[datetime] = None
    ) -> CurrentTransits:
        """
        ...
        active_effects = self._analyze_transit_effects(
            birth_data, jupiter_transit, saturn_transit, rahu_transit, ketu_transit
        )
    ...
    def _calculate_jupiter_transit(self, birth_data: Dict[str, Any], current_date: datetime) -> TransitPosition:
        ...
        house_in_natal = self._determine_transit_house(current_sign, birth_data.get("ascendant_sign", "Aries"))

        # Determine aspects to natal planets
        aspects_planets = self._calculate_aspects("Jupiter", house_in_natal, birth_data.get("planets", {}))
    ...
    def _calculate_saturn_transit(self, birth_data: Dict[str, Any], current_date: datetime) -> TransitPosition:
        ...
        aspects_planets = self._calculate_aspects("Saturn", house_in_natal, birth_data.get("planets", {}))
    ...
    def _calculate_rahu_transit(self, birth_data: Dict[str, Any], current_date: datetime) -> TransitPosition:
        ...
        aspects_planets = self._calculate_aspects("Rahu", house_in_natal, birth_data.get("planets", {}))
    ...
    def _calculate_ketu_transit(self, birth_data: Dict[str, Any], current_date: datetime) -> TransitPosition:
        ...
        aspects_planets = self._calculate_aspects("Ketu", house_in_natal, birth_data.get("planets", {}))
    ...
    def _determine_transit_house(self, transit_sign: str, ascendant_sign: str) -> int:
    ...
    def _calculate_aspects(self, transit_planet: str, transit_house: int, natal_planets: Dict[str, Any]) -> List[str]:
    ...
    def _generate_transit_recommendations(self, effects: List[TransitEffect]) -> List[str]:
        ...
        if jupiter_effects:
            recommendations.append("Leverage Jupiter's beneficial influence for expansion and growth opportunities")

        # Saturn recommendations
        saturn_effects = [e for e in effects if e.transit_planet == "Saturn"]
        if saturn_effects:
            recommendations.append("Exercise patience and discipline during Saturn transit - focus on long-term goals")

        # Rahu recommendations
        rahu_effects = [e for e in effects if e.transit_planet == "Rahu"]
        if rahu_effects:
            recommendations.append("Be mindful of Rahu's intensifying influence - avoid obsession and maintain balance")

...
class CacheItem(Generic[T]):
    ...
    def __init__(self, key: str, value: T, expiry: Optional[datetime] = None, cost: float = 1.0):
        self.key = key
        self.value = value
        self.expiry = expiry
        self.cost = cost
...
class AdvancedCache(Generic[T]):
    ...
    def _calculate_memory_usage(self) -> float:
        ...
        total_size = sum(sys.getsizeof(item.value) for item in self.cache.values())
    ...
    async def get(self, key: str) -> Optional[T]:
        ...
        with self.lock:
            ...
            self.metrics.avg_access_time = (
                self.metrics.avg_access_time * (self.metrics.hits - 1) + (time.time() - start_time)
            ) / self.metrics.hits
    ...
    async def put(self, key: str, value: T, ttl: Optional[int] = None, cost: float = 1.0) -> bool:
        """Put value in cache"""
        with self.lock:
            # Check memory limit
            ...
            # Calculate expiry
            expiry = (
                datetime.now() + timedelta(seconds=ttl) if ttl else datetime.now() + timedelta(seconds=self.default_ttl)
            )
    ...
    async def _evict(self) -> None:
        ...
        if self.eviction_policy == "lru":
            ...
            # Highest Cost
            max_cost_key = max(self.cache.items(), key=lambda x: x[1].cost)[0]
            del self.cache[max_cost_key]
        else:  # adaptive
            # Consider multiple factors
            scores = {key: self._calculate_item_score(item) for key, item in self.cache.items()}
    ...
    async def cleanup_expired(self) -> int:
        ...
        with self.lock:
            ...
            expired_keys = [key for key, item in self.cache.items() if item.expiry and current_time > item.expiry]
...
def cached(cache: AdvancedCache, ttl: Optional[int] = None, cost: float = 1.0):

...
class CacheEngine:
    ...
    async def set(
        self, key: str, value: Any, ttl: Optional[int] = None, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """Set value in cache"""
        if not self._initialized:
            await self.initialize()
        ...
        if self.config.backend == CacheBackend.MEMORY:
            ...
            await self._redis.set(full_key, self._serialize(value), expire=ttl or self.config.ttl)
    ...
    async def _apply_cache_strategy(self) -> None:
        ...
        if self.config.strategy == CacheStrategy.LRU:
            ...
            now = datetime.utcnow()
            expired = [k for k, v in self._cache.items() if (now - v.created_at).total_seconds() > v.ttl]
            for key in expired:
                del self._cache[key]
...
cache_engine = CacheEngine(
    CacheConfig(backend=CacheBackend.MEMORY, strategy=CacheStrategy.LRU, ttl=300, max_size=1000, namespace="kundli")
)

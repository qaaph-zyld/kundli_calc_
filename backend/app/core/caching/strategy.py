...
class CacheManager:
    ...
    def _initialize_default_policies(self) -> None:
        ...
        self.add_policy(
            CachePolicy(
                ...
                ttl=300,  # 5 minutes
                key_prefix="kundli",
                invalidation_events=["kundli_update", "kundli_delete"],
            )
        )
        ...
        self.add_policy(
            CachePolicy(
                ...
                ttl=600,  # 10 minutes
                key_prefix="pattern",
                invalidation_events=["pattern_update", "pattern_delete"],
                dependencies=["kundli"],
            )
        )
        ...
        self.add_policy(
            CachePolicy(
                ...
                ttl=1800,  # 30 minutes
                key_prefix="user",
                invalidation_events=["user_update", "user_delete"],
            )
        )
        ...
        self.add_policy(
            CachePolicy(
                ...
                ttl=3600,  # 1 hour
                key_prefix="system",
                invalidation_events=["system_update", "config_update"],
            )
        )
    ...
    async def invalidate(self, data_type: DataType, event: str, key: Optional[str] = None) -> None:
        ...
        if event in policy.invalidation_events:
            if key:
                ...
                if self.cache.config.backend == "redis":
                    ...
                    keys_to_delete = [k for k in self.cache._cache.keys() if k.startswith(f"{policy.key_prefix}:")]
    ...
    def cached_kundli(self, ttl: Optional[int] = None):
        ...
        return cached(
            ...
            key_builder=lambda *args, **kwargs: self.build_key(
                DataType.KUNDLI, str(kwargs.get("kundli_id") or args[0])
            ),
        )
    ...
    def cached_pattern(self, ttl: Optional[int] = None):
        ...
        return cached(
            ttl=ttl or policy.ttl,
            key_builder=lambda *args, **kwargs: self.build_key(
                DataType.PATTERN, str(kwargs.get("kundli_id") or args[0])
            ),
        )
    ...
    def cached_user(self, ttl: Optional[int] = None):
        ...
        return cached(
            ttl=ttl or policy.ttl,
            key_builder=lambda *args, **kwargs: self.build_key(DataType.USER, str(kwargs.get("user_id") or args[0])),
        )
    ...
    def cached_system(self, ttl: Optional[int] = None):
        ...
        return cached(
            ttl=ttl or policy.ttl,
            key_builder=lambda *args, **kwargs: self.build_key(DataType.SYSTEM, str(kwargs.get("key") or args[0])),
        )

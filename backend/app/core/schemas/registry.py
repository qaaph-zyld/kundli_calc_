...
class SchemaRegistry:
    ...
    def get_schema(self, name: str, version: SchemaVersion = SchemaVersion.LATEST) -> Optional[Type[BaseModel]]:
        """Get schema by name and version"""
        return self._schemas.get(name, {}).get(version)

    def get_optimizer(self, name: str) -> Optional[SchemaOptimizer]:
        """Get schema optimizer by name"""
        return self._optimizers.get(name)

    async def optimize_response(self, name: str, data: dict, format: ResponseFormat = ResponseFormat.JSON) -> dict:

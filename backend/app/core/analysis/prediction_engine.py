...
class PredictionEngine:
    ...
    async def generate_predictions(
        ...
        try:
            ...
            correlations = await self.correlation_analyzer.analyze_correlations(chart_data)
            ...
            predictions = await self._generate_combined_predictions(
                patterns, correlations, yogas, scopes or list(PredictionScope), domains or list(PredictionDomain)
            )
    ...
    async def _generate_combined_predictions(
        ...
        for domain in domains:
            ...
            domain_correlations = [c for c in correlations if self._map_correlation_to_domain(c) == domain]

            # Generate domain-specific predictions
            for scope in scopes:
                prediction = self._create_prediction(
                    domain, scope, domain_patterns.get(domain, []), domain_correlations, domain_yogas
                )
    ...
    def _create_prediction(
        ...
        strength = self._calculate_combined_strength(patterns, correlations, yogas)
        ...
        description = self._fill_template(template, domain, scope, patterns, correlations, yogas)
        ...
        return PredictionResult(
            ...
            confidence=self._calculate_confidence(patterns, correlations, yogas),
            timing=timing,
            factors=factors,
            strength=strength,
    ...
    def _calculate_combined_strength(
        self, patterns: List[PatternMatch], correlations: List[CorrelationResult], yogas: List[YogaResult]
    ) -> float:
    ...
    def _calculate_timing(
        self, scope: PredictionScope, patterns: List[PatternMatch], yogas: List[YogaResult]
    ) -> Dict[str, Any]:
        """Calculate timing information for prediction"""
        timing = {"start": datetime.now(), "duration": None, "peak": None}
    ...
    def _group_by_domain(self, patterns: List[PatternMatch]) -> Dict[PredictionDomain, List[PatternMatch]]:
    ...
    def _map_pattern_to_domain(self, pattern: PatternMatch) -> Optional[PredictionDomain]:
    ...
    def _map_yoga_to_domain(self, yoga: YogaResult) -> PredictionDomain:
    ...
    def _map_correlation_to_domain(self, correlation: CorrelationResult) -> PredictionDomain:
    ...
    def get_prediction_metrics(self) -> Dict[str, Any]:
        ...
        if self.predictions:
            ...
            # Calculate averages
            metrics["average_probability"] = np.mean([p.probability for p in self.predictions])
            metrics["average_confidence"] = np.mean([p.confidence for p in self.predictions])

...
class BhavaAnalyzer:
    ...
    def analyze_bhava(
        ...
        aspects = self._calculate_aspects_to_house(house_sign, planet_positions)

        # Check Papa/Shubha Kartari
        papa_kartari, shubha_kartari = self._check_kartari_yoga(house_sign, planet_positions)

        # Calculate overall strength
        strength_pct, strength_level = self._calculate_bhava_strength(
            lord_strength, len(planets_in_house), len(aspects), ashtakavarga_bindus, house_number
        )
        ...
        recommendations = self._generate_recommendations(house_number, strength_level, lord, planets_in_house)
    ...
    def _calculate_aspects_to_house(self, house_sign: int, planet_positions: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        for planet, lon in planet_positions.items():
            ...
            if (planet_sign + 6) % 12 == house_sign:
                aspects.append({"planet": planet, "aspect_type": "7th (Opposition)", "strength": 1.0})
    ...
    def _check_kartari_yoga(self, house_sign: int, planet_positions: Dict[str, float]) -> Tuple[bool, bool]:
        ...
        papa_kartari = any(p in malefics for p in prev_planets) and any(p in malefics for p in next_planets)

        shubha_kartari = any(p in benefics for p in prev_planets) and any(p in benefics for p in next_planets)

        return papa_kartari, shubha_kartari

    def _calculate_bhava_strength(
        self, lord_strength: float, planet_count: int, aspect_count: int, av_bindus: Optional[int], house_number: int
    ...
    def _generate_recommendations(
        self, house_num: int, strength: BhavaStrength, lord: str, planets: List[str]
    ) -> List[str]:
    ...
    def analyze_all_bhavas(
        ...
        for house_num in range(1, 13):
            ...
            results[house_num] = self.analyze_bhava(
                house_num, ascendant, planet_positions, planet_strengths, av_bindus, chara_karakas
            )
...
def create_comprehensive_bhava_report(
    ...
    all_bhavas = analyzer.analyze_all_bhavas(
        ascendant, planet_positions, planet_strengths, sarvashtakavarga, chara_karakas
    )

    # Identify strongest and weakest houses
    sorted_bhavas = sorted(all_bhavas.items(), key=lambda x: x[1].strength_percentage, reverse=True)
    ...
    return {
        ...
        "strongest_houses": [
            {"house": num, "name": result.house_name, "strength": result.strength_percentage}
            for num, result in strongest
        ],
        "weakest_houses": [
            {"house": num, "name": result.house_name, "strength": result.strength_percentage, "needs_remedies": True}

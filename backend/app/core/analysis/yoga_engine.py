...
class YogaEngine:
    ...
    def _initialize_yoga_definitions(self) -> None:
        ...
        self.raja_yoga_conditions = {
            "kendra_trikona": {"lords": ["any"], "houses": [1, 4, 7, 10, 5, 9], "min_planets": 2, "strength": 0.8},
            "mutual_aspect": {"planets": ["Jupiter", "Venus"], "aspect_type": "trine", "strength": 0.9},
        }

        self.dhana_yoga_conditions = {
            "house_2_11": {"planets": ["Jupiter", "Venus"], "houses": [2, 11], "strength": 0.7},
            "benefic_aspect": {
        ...
        self.mahapurusha_conditions = {
            "ruchaka": {"planet": "Mars", "houses": [1, 4, 7, 10], "own_sign": True, "strength": 0.9},
            "bhadra": {"planet": "Mercury", "houses": [1, 4, 7, 10], "own_sign": True, "strength": 0.9},
            "hamsa": {"planet": "Jupiter", "houses": [1, 4, 7, 10], "own_sign": True, "strength": 0.9},
            "malavya": {"planet": "Venus", "houses": [1, 4, 7, 10], "own_sign": True, "strength": 0.9},
            "sasa": {"planet": "Saturn", "houses": [1, 4, 7, 10], "own_sign": True, "strength": 0.9},
        }

    async def analyze_yogas(self, chart_data: Dict[str, Any]) -> List[YogaResult]:
    ...
    async def _check_raja_yogas(self, chart_data: Dict[str, Any]) -> List[YogaResult]:
            ...
            if len(house_planets) >= kendra_trikona["min_planets"]:
                results.append(
                    YogaResult(
                        ...
                        houses=[house],
                        effects={"power": 0.8, "status": 0.7, "wealth": 0.6},
        ...
        for aspect in aspects:
            if aspect["type"] == mutual["aspect_type"] and set(aspect["planets"]).issubset(set(mutual["planets"])):
                results.append(
                    YogaResult(
                        yoga_type=YogaType.RAJAYOGA,
                        ...
                        houses=[],
                        effects={"power": 0.9, "status": 0.8, "wealth": 0.7},
                        timestamp=datetime.now(),
                    )
                )
    ...
    async def _check_dhana_yogas(self, chart_data: Dict[str, Any]) -> List[YogaResult]:
        ...
        for house in house_condition["houses"]:
            house_planets = self._get_planets_in_house(planets, house)
            matching_planets = [p for p in house_planets if p in house_condition["planets"]]
            ...
            if matching_planets:
                results.append(
                    YogaResult(
                        ...
                        houses=[house],
                        effects={"wealth": 0.8, "prosperity": 0.7, "abundance": 0.6},
                        timestamp=datetime.now(),
                    )
                )
        ...
        for aspect in aspects:
            if aspect["type"] in benefic["aspect_type"] and any(p in benefic["planets"] for p in aspect["planets"]):
                results.append(
                    YogaResult(
                        yoga_type=YogaType.DHANYOGA,
                        ...
                        planets=aspect["planets"],
                        houses=[],
                        effects={"wealth": 0.7, "prosperity": 0.8, "abundance": 0.7},
                        timestamp=datetime.now(),
                    )
                )
    ...
    async def _check_mahapurusha_yogas(self, chart_data: Dict[str, Any]) -> List[YogaResult]:
        ...
        for yoga_name, condition in self.mahapurusha_conditions.items():
            ...
            if house in condition["houses"] and (not condition["own_sign"] or planet_data.get("in_own_sign", False)):
                results.append(
                    YogaResult(
                        ...
                        planets=[planet],
                        houses=[house],
                        effects={"personality": 0.9, "success": 0.8, "leadership": 0.7},
                        timestamp=datetime.now(),
                    )
                )
    ...
    async def _check_special_yogas(self, chart_data: Dict[str, Any]) -> List[YogaResult]:
        ...
        if self._check_gajakesari(planets):
            results.append(
                YogaResult(
                    ...
                    houses=[],
                    effects={"fortune": 0.8, "wisdom": 0.7, "success": 0.7},
                    timestamp=datetime.now(),
                )
            )
        ...
        if viparita:
            results.append(
                YogaResult(
                    ...
                    effects={"transformation": 0.9, "power": 0.8, "success": 0.8},
                    timestamp=datetime.now(),
                )
            )
    ...
    def _get_planets_in_house(self, planets: Dict[str, Any], house: int) -> List[str]:
        """Get list of planets in a specific house"""
        return [planet for planet, data in planets.items() if data.get("house") == house]

    def _check_gajakesari(self, planets: Dict[str, Any]) -> bool:
    ...
    def _check_viparita(self, planets: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if result_planets:
            return {"planets": result_planets, "houses": result_houses}
    ...
    def get_yoga_metrics(self) -> Dict[str, Any]:
        ...
        if self.detected_yogas:
            ...
            # Calculate averages
            metrics["average_strength"] = sum(y.strength for y in self.detected_yogas) / len(self.detected_yogas)

            metrics["average_confidence"] = sum(y.confidence for y in self.detected_yogas) / len(self.detected_yogas)

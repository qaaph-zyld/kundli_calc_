...
class PatternDetector:
    ...
    def _initialize_patterns(self) -> None:
        ...
        self.signatures["mutual_reception"] = PatternSignature(
            ...
            pattern_type=PatternType.PLANETARY,
            conditions=[{"type": "planet_exchange", "min_planets": 2, "exchange_type": "rulership"}],
        ...
        self.signatures["trikona"] = PatternSignature(
            ...
            conditions=[{"type": "house_angle", "angle": 120, "tolerance": 5}],
        ...
        self.signatures["kendra"] = PatternSignature(
            name="Kendra",
            pattern_type=PatternType.HOUSE,
            conditions=[{"type": "house_angle", "angle": 90, "tolerance": 5}],
            effects={"power": 0.9},
        )
        ...
        self.signatures["grand_trine"] = PatternSignature(
            ...
            conditions=[{"type": "aspect_chain", "aspect": "trine", "min_planets": 3}],
        ...
        self.signatures["planetary_hour"] = PatternSignature(
            ...
            conditions=[{"type": "temporal_match", "factor": "hour_ruler", "match_type": "planet"}],
            effects={"timing": 0.7},
        )

    async def detect_patterns(self, chart_data: Dict[str, Any]) -> List[PatternMatch]:
    ...
    async def _match_signature(self, signature: PatternSignature, chart_data: Dict[str, Any]) -> List[PatternMatch]:
        """Match a specific pattern signature"""
        matches = []

        try:
            if signature.pattern_type == PatternType.PLANETARY:
                matches.extend(await self._detect_planetary_patterns(signature, chart_data))
            elif signature.pattern_type == PatternType.HOUSE:
                matches.extend(await self._detect_house_patterns(signature, chart_data))
            elif signature.pattern_type == PatternType.ASPECT:
                matches.extend(await self._detect_aspect_patterns(signature, chart_data))
            elif signature.pattern_type == PatternType.TEMPORAL:
                matches.extend(await self._detect_temporal_patterns(signature, chart_data))

        except Exception as e:
            self.logger.error(f"Error detecting {signature.name} pattern: {str(e)}")

        return matches

    async def _detect_planetary_patterns(
        self, signature: PatternSignature, chart_data: Dict[str, Any]
    ) -> List[PatternMatch]:
        ...
        for condition in signature.conditions:
            if condition["type"] == "planet_sign":
                ...
                for planet, sign in condition["relations"].items():
                    if planet in planets and planets[planet].get("sign") == sign:
                        matched_planets.append({"planet": planet, "sign": sign})

                if matched_planets:
                    strength = len(matched_planets) / len(condition["relations"])
                ...
                exchanges = self._find_planet_exchanges(planets, condition["exchange_type"])
    ...
    async def _detect_house_patterns(
        ...
        for condition in signature.conditions:
            if condition["type"] == "house_angle":
                ...
                house_groups = self._find_angular_relationships(houses, angle, tolerance)
    ...
    async def _detect_aspect_patterns(
        ...
        for condition in signature.conditions:
            if condition["type"] == "aspect_chain":
                ...
                chains = self._find_aspect_chains(aspects, condition["aspect"], condition["min_planets"])
        ...
        for condition in signature.conditions:
            if condition["type"] == "temporal_match":
                ...
                if condition["factor"] in temporal:
                    ...
                    if self._check_temporal_match(factor_value, condition["match_type"]):
                        matches.append(
                            PatternMatch(
                                pattern_type=signature.pattern_type,
                                name=signature.name,
                                confidence=0.75,
                                components=[{"factor": condition["factor"], "value": factor_value}],
                                timestamp=datetime.now(),
                                strength=0.75,
                                effects=signature.effects,
    ...
    def _find_planet_exchanges(self, planets: Dict[str, Any], exchange_type: str) -> List[List[Dict[str, Any]]]:
        ...
        for p1_name, p1_data in planets.items():
            ...
            for p2_name, p2_data in planets.items():
                if p1_name != p2_name and self._check_exchange(p1_data, p2_data, exchange_type):
                    exchange_group.extend([{"planet": p1_name, "data": p1_data}, {"planet": p2_name, "data": p2_data}])
    ...
    def _find_angular_relationships(
        self, houses: Dict[int, Any], target_angle: float, tolerance: float
            ...
            for h2 in houses:
                if h1 != h2:
                    ...
                    if abs(angle - target_angle) <= tolerance:
                        group.extend([{"house": h1, "data": houses[h1]}, {"house": h2, "data": houses[h2]}])
                        processed.add(h1)
                        processed.add(h2)
    ...
    def _find_aspect_chains(
        self, aspects: List[Dict[str, Any]], aspect_type: str, min_planets: int
    ) -> List[List[Dict[str, Any]]]:
        """Find chains of aspects"""
        chains = []
        ...
        def dfs(planet: str, chain: List[str]) -> None:
            if len(chain) >= min_planets:
                chains.append([{"planet": p} for p in chain])
    ...
    def _check_exchange(self, p1_data: Dict[str, Any], p2_data: Dict[str, Any], exchange_type: str) -> bool:
        """Check if two planets are in exchange"""
        if exchange_type == "rulership":
            return p1_data.get("rules") == p2_data.get("sign") and p2_data.get("rules") == p1_data.get("sign")
        return False

    def _calculate_house_angle(self, house1: int, house2: int) -> float:
        """Calculate angle between houses"""
        return abs((house2 - house1) * 30)

    def _check_temporal_match(self, value: Any, match_type: str) -> bool:
        """Check temporal factor match"""
        if match_type == "planet":
            return isinstance(value, str)
        return False

    async def analyze_pattern_sequence(self, timeframe: Optional[Tuple[datetime, datetime]] = None) -> Dict[str, Any]:
        """Analyze sequence of detected patterns"""
        patterns = self.detected_patterns
        if timeframe:
            start, end = timeframe
            patterns = [p for p in patterns if start <= p.timestamp <= end]
        ...
        for pattern in patterns:
            ...
            # Track strengths
            analysis["pattern_strengths"][pattern.name].append(pattern.strength)
            ...
            if pattern.pattern_type == PatternType.COMBINED:
                for comp in pattern.components:
                    if "pattern" in comp:
                        analysis["common_combinations"][comp["pattern"]] += 1
    ...
    def get_pattern_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pattern_types": {
                pt.value: len([p for p in self.detected_patterns if p.pattern_type == pt]) for pt in PatternType
            },
            "average_confidence": (
                np.mean([p.confidence for p in self.detected_patterns]) if self.detected_patterns else 0
            ),
            "latest_detection": max((p.timestamp for p in self.detected_patterns), default=None),
        }

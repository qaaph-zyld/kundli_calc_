...
class TraditionalJyotishMaster:
    ...
    def calculate_complete_analysis(
        ...
        ashtakavarga = calculate_complete_ashtakavarga(
            planet_positions,
            ascendant,
            apply_reductions=apply_ashtakavarga_reductions
        )
        
        # 2. Calculate Shadbala (planetary strength)
        print("Calculating Shadbala...")
        shadbala_results = self._calculate_shadbala_all_planets(
            planet_positions,
            ascendant,
            birth_datetime
        )
        ...
        dasha_info = self.dasha.calculate_all_dasha_levels(
            birth_datetime,
            moon_longitude,
            include_sookshma=False,
            include_prana=False
        )
        
        current_dasha = self.dasha.get_current_dasha(
            birth_datetime,
            moon_longitude,
            datetime.now()
        )
        ...
        weak_planets = [
            p for p, data in shadbala_results.items()
        ...
        mantra_charity = create_complete_remedial_plan(
            weak_planets,
            current_maha
        )
        ...
        areas_needing_attention = self._identify_areas_needing_attention(
            bhava_analysis,
            weak_planets,
            shadbala_results
        )
    ...
    def _calculate_shadbala_all_planets(
        ...
        for planet in ['Sun', 'Moon', 'Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn']:
            ...
            shadbala = self.shadbala.calculate_shadbala(
                planet,
                house,
                speed,
                aspects,
    ...
    def _identify_key_strengths(
        ...
        # Strong planets
        strong_planets = [
            p for p, data in shadbala.items()

...
class ValidationFramework:
    ...
    def _load_rules(self):
        ...
        self.add_rule(
            ValidationRule(
                ...
                parameters={"min_year": 1900, "max_year": 2100},
            )
        )
        ...
        self.add_rule(
            ValidationRule(
                ...
                parameters={"position_range": (0, 360), "speed_range": (-5, 5)},
            )
        )
        ...
        self.add_rule(
            ValidationRule(
                ...
                level=ValidationLevel.STANDARD,
                scope=ValidationScope.CALCULATION,
                parameters={"valid_systems": ["Placidus", "Koch", "Equal", "Whole Sign"]},
            )
        )
        ...
        self.add_rule(
            ValidationRule(
                ...
                parameters={"max_orb": 10, "valid_aspects": [0, 60, 90, 120, 180]},
            )
        )
    ...
    async def _apply_rule(self, rule: ValidationRule, data: Dict[str, Any]) -> Optional[ValidationError]:
        ...
        try:
            ...
            return ValidationError(code=rule.code, message=f"Validation error: {str(e)}", severity=rule.severity)
        return None
    ...
    def _validate_date(self, data: Dict[str, Any], rule: ValidationRule) -> Optional[ValidationError]:
        ...
        try:
            ...
            if not date_str:
                return ValidationError(code=rule.code, message="Date is required", field="date")
            ...
            if not min_year <= date.year <= max_year:
                return ValidationError(
                    code=rule.code, message=f"Year must be between {min_year} and {max_year}", field="date"
                )
        except ValueError:
            return ValidationError(code=rule.code, message="Invalid date format. Use YYYY-MM-DD", field="date")
        return None
    ...
    def _validate_coordinates(self, data: Dict[str, Any], rule: ValidationRule) -> Optional[ValidationError]:
        ...
        if lat is None or lon is None:
            return ValidationError(code=rule.code, message="Latitude and longitude are required", field="coordinates")
        ...
        if not lat_min <= lat <= lat_max:
            return ValidationError(
                code=rule.code, message=f"Latitude must be between {lat_min} and {lat_max}", field="latitude"
            )

        if not lon_min <= lon <= lon_max:
            return ValidationError(
                code=rule.code, message=f"Longitude must be between {lon_min} and {lon_max}", field="longitude"
            )
    ...
    def _validate_planet_positions(self, data: Dict[str, Any], rule: ValidationRule) -> Optional[ValidationError]:
        ...
        if not planets:
            return ValidationError(code=rule.code, message="No planetary positions found", field="planets")
        ...
        for planet, details in planets.items():
            ...
            if position is None or not pos_min <= position <= pos_max:
                return ValidationError(
                    code=rule.code, message=f"Invalid position for {planet}", field=f"planets.{planet}.longitude"
                )

            if speed is not None and not speed_min <= speed <= speed_max:
                return ValidationError(
                    code=rule.code, message=f"Invalid speed for {planet}", field=f"planets.{planet}.speed"
                )
    ...
    def _validate_house_system(self, data: Dict[str, Any], rule: ValidationRule) -> Optional[ValidationError]:
        ...
        if not house_system:
            return ValidationError(code=rule.code, message="House system is required", field="house_system")
    ...
    def _validate_aspects(self, data: Dict[str, Any], rule: ValidationRule) -> Optional[ValidationError]:
        ...
        for aspect in aspects:
            ...
            if orb is None or orb > max_orb:
                return ValidationError(
                    code=rule.code, message=f"Invalid orb. Must be less than {max_orb} degrees", field="aspects.orb"
                )

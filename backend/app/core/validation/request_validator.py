...
class KundliRequest(BaseModel):
    ...
    ayanamsa: AyanamsaSystem = Field(default=AyanamsaSystem.LAHIRI, description="Ayanamsa system to use")
    house_system: HouseSystem = Field(default=HouseSystem.WHOLE_SIGN, description="House system to use")
    chart_types: List[ChartType] = Field(default=[ChartType.RASI], description="Types of charts to calculate")
    calculation_type: CalculationType = Field(default=CalculationType.BASIC, description="Level of calculation detail")
    language: str = Field(default="en", description="Language for predictions and interpretations")
...
class MatchingRequest(BaseModel):
    ...
    match_factors: Optional[List[str]] = Field(
        default=["ashtakoot", "dashakoot", "guna_milan"], description="Factors to consider for matching"
    )
...
class PredictionRequest(KundliRequest):
    ...
    prediction_start: Optional[str] = Field(None, description="Start date for predictions (YYYY-MM-DD)")
    prediction_end: Optional[str] = Field(None, description="End date for predictions (YYYY-MM-DD)")
    domains: Optional[List[str]] = Field(
        default=["general", "career", "relationship", "health", "finance"], description="Domains for predictions"
    )
...
class DashaRequest(KundliRequest):
    ...
    dasha_system: str = Field(default="vimshottari", description="Dasha system to use")
    levels: int = Field(default=2, description="Number of dasha levels to calculate", ge=1, le=4)
...
class MuhurtaRequest(BaseModel):
    ...
    duration_hours: Optional[float] = Field(None, description="Required duration in hours", gt=0)

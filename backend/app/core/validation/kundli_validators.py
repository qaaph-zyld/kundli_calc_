...
from .validators import BusinessValidator, DataValidator, ValidationRule, ValidationSeverity, ValidationType
...
class KundliValidationRequest(BaseModel):
    ...
    validation_types: Optional[List[ValidationType]] = Field(
        default=[ValidationType.DATA, ValidationType.BUSINESS], description="Types of validation to perform"
    )
...
async def validate_kundli_data(request: KundliValidationRequest) -> KundliValidationResponse:
    ...
    # Initialize response
    response = KundliValidationResponse(is_valid=True, metadata={"timestamp": datetime.utcnow()})
        ...
        for result in data_results:
            if not result.passed:
                ...
                if result.rule.severity == ValidationSeverity.ERROR:
                    response.errors.append({"type": "data", "rule": result.rule.name, "message": result.message})
                else:
                    response.warnings.append({"type": "data", "rule": result.rule.name, "message": result.message})
    ...
    if ValidationType.BUSINESS in request.validation_types:
        ...
        for result in business_results:
            if not result.passed:
                if result.rule.severity == ValidationSeverity.ERROR:
                    response.is_valid = False
                    response.errors.append({"type": "business", "rule": result.rule.name, "message": result.message})
                else:
                    response.warnings.append({"type": "business", "rule": result.rule.name, "message": result.message})

    # Add validation metrics to metadata
    response.metadata.update(
        {"data_metrics": data_validator.metrics.__dict__, "business_metrics": business_validator.metrics.__dict__}
    )

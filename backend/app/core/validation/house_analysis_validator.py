...
class HouseAnalysisValidator:
    ...
    async def validate_house_analysis(self, test_cases: List[Dict[str, Any]]) -> ValidationMetrics:
        """
        ...
        try:
            ...
            validation_status = self._determine_validation_status(
                accuracy, precision, recall, execution_time, resource_usage
            )
    ...
    async def _validate_single_case(self, test_case: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            is_valid = self._validate_strength_result(
                result.total_strength, expected_strength, tolerance=test_case.get("tolerance", 5.0)
            )
    ...
    def _validate_strength_result(self, actual: float, expected: float, tolerance: float) -> bool:
    ...
    def _determine_validation_status(
        self, accuracy: float, precision: float, recall: float, execution_time: float, resource_usage: Dict[str, float]
    ) -> str:
        ...
        if (
            ...
            and execution_time <= self.thresholds.max_execution_time
            and resource_usage["memory_mb"] <= self.thresholds.max_memory_usage
        ):
            return "PASSED"

        if (
            accuracy >= self.thresholds.min_accuracy * 0.9
            and precision >= self.thresholds.min_precision * 0.9
            and recall >= self.thresholds.min_recall * 0.9
        ):
            return "WARNING"

        return "FAILED"

    def get_validation_history(self, limit: Optional[int] = None) -> List[ValidationMetrics]:
    ...
    def get_trend_analysis(self) -> Dict[str, Any]:
        ...
        metrics = np.array([[m.accuracy, m.precision, m.recall, m.execution_time] for m in self.validation_history])

        return {
            "accuracy_trend": {
                "mean": float(np.mean(metrics[:, 0])),
                "std": float(np.std(metrics[:, 0])),
                "trend": "improving" if metrics[-1, 0] > np.mean(metrics[:-1, 0]) else "declining",
            },
            "precision_trend": {
                "mean": float(np.mean(metrics[:, 1])),
                "std": float(np.std(metrics[:, 1])),
                "trend": "improving" if metrics[-1, 1] > np.mean(metrics[:-1, 1]) else "declining",
            },
            "recall_trend": {
                "mean": float(np.mean(metrics[:, 2])),
                "std": float(np.std(metrics[:, 2])),
                "trend": "improving" if metrics[-1, 2] > np.mean(metrics[:-1, 2]) else "declining",
            },
            "performance_trend": {
                "mean": float(np.mean(metrics[:, 3])),
                "std": float(np.std(metrics[:, 3])),
                "trend": "improving" if metrics[-1, 3] < np.mean(metrics[:-1, 3]) else "declining",
            },
        }

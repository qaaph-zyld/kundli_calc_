...
class DeploymentManager:
    ...
    async def _deploy_docker(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.docker_client.networks.create(self.config.network, driver="bridge")
    ...
    async def _deploy_docker_service(self, name: str, service: ServiceConfig) -> Dict[str, Any]:
    ...
    def _build_docker_image(self, service: ServiceConfig) -> None:
        ...
        self.docker_client.images.build(path=".", dockerfile=str(dockerfile_path), tag=f"{service.image}:{service.tag}")
    ...
    def _generate_k8s_manifests(self) -> Dict[str, Any]:
        ...
        for name, service in self.config.services.items():
                            ...
                            "containers": [
                                {
                                    ...
                                    "env": [
                                        {"name": k, "value": v}
                                        for k, v in {**self.config.global_env_vars, **service.env_vars}.items()
                                    ],
                                    "resources": {
                                        "requests": {"cpu": service.resources.cpu, "memory": service.resources.memory},
                                        "limits": {"cpu": service.resources.cpu, "memory": service.resources.memory},
                                    },
                                    "livenessProbe": {
                                        "httpGet": {
            ...
            service_manifest = {
                ...
                "spec": {
                    "selector": {"app": name},
                    "ports": [{"protocol": "TCP", "port": service.port, "targetPort": service.port}],
                },
            }
    ...
    async def _deploy_serverless(self) -> Dict[str, Any]:
        ...
        result = subprocess.run(
            ["serverless", "deploy", "--stage", self.config.environment], capture_output=True, text=True
        )
        ...
        return {
            ...
            "functions": {
                name: {
                    ...
                    "events": [{"http": {"path": f"/{name}", "method": "any"}}],
    ...
    async def _deploy_bare_metal(self) -> Dict[str, Any]:
        ...
        for name, service in self.config.services.items():
            ...
            if src_dir.exists():
                shutil.copytree(src_dir, service_dir, dirs_exist_ok=True)
...
"""
            ...
            result = subprocess.run(["systemctl", "start", f"{name}.service"], capture_output=True, text=True)

...
class ServiceRegistry:
    ...
    async def watch_service(self, service_name: str, callback: Callable[[List[ServiceInstance]], None]) -> None:
        """Watch service for changes"""
        raise NotImplementedError

    async def unwatch_service(self, service_name: str, callback: Callable[[List[ServiceInstance]], None]) -> None:
        """Unwatch service"""
...
class ConsulRegistry(ServiceRegistry):
    ...
    def __init__(self, config: DiscoveryConfig):
        ...
        self.client = consul.aio.Consul(
            host=config.registry_urls[0].split(":")[0], port=int(config.registry_urls[0].split(":")[1])
        )
    ...
    async def register(self, instance: ServiceInstance) -> None:
        ...
        try:
            ...
            check = {
                ...
                "timeout": "5s",
                "deregister_critical_service_after": f"{self.config.deregistration_delay}s",
            }
            ...
            await self.client.agent.service.register(
                ...
                meta={
                    ...
                    "endpoints": json.dumps([endpoint.dict() for endpoint in instance.metadata.endpoints]),
                },
            )
    ...
    async def deregister(self, service_name: str, instance_id: str) -> None:
        ...
        try:
            ...
            if service_name in self._instances and instance_id in self._instances[service_name]:
            ...
            for service in services:
                ...
                instance = ServiceInstance(
                    ...
                    metadata=ServiceMetadata(
                        ...
                        endpoints=[ServiceEndpoint(**endpoint) for endpoint in endpoints],
                    ),
                    health_check_url=f"http://{service_data['Address']}:" f"{service_data['Port']}/health",
                )
    ...
    async def watch_service(self, service_name: str, callback: Callable[[List[ServiceInstance]], None]) -> None:
        ...
        async def watch_loop():
            ...
            while True:
                try:
                    index, services = await self.client.health.service(service_name, index=index, passing=True)

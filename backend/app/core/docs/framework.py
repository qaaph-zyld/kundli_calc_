...
class DocGenerator:
    ...
    def _setup_paths(self) -> None:
        ...
        # Create directories
        for path in [self.docs_path, self.api_docs_path, self.code_docs_path, self.arch_docs_path]:
    ...
    def generate_api_docs(self) -> Documentation:
        ...
        # Endpoints section
        endpoints_section = DocSection(title="Endpoints", content="# API Endpoints\n\n", subsections=[])
        ...
        for path, path_item in openapi_schema["paths"].items():
            for method, operation in path_item.items():
                endpoint_doc = DocSection(
                    ...
                    metadata={"method": method, "path": path, "tags": operation.get("tags", [])},
                )
        ...
        # Models section
        models_section = DocSection(title="Data Models", content="# Data Models\n\n", subsections=[])

        for schema_name, schema in openapi_schema["components"]["schemas"].items():
            model_doc = DocSection(
                title=schema_name, content=self._format_model_doc(schema_name, schema), metadata={"schema": schema}
            )
        ...
        api_docs = Documentation(
            ...
            sections=sections,
            metadata={"generated_at": datetime.utcnow().isoformat(), "openapi_version": openapi_schema["openapi"]},
        )
    ...
    def generate_code_docs(self, source_dir: Path) -> Documentation:
        ...
        code_docs = Documentation(
            ...
            format=DocFormat.MARKDOWN,
            sections=sections,
            metadata={"generated_at": datetime.utcnow().isoformat(), "source_dir": str(source_dir)},
        )
...
3. Infrastructure
   ...
   - Monitoring""",
                subsections=[
                    DocSection(title="Backend Architecture", content=self._generate_backend_arch_doc()),
                    DocSection(title="Data Flow", content=self._generate_data_flow_doc()),
                    DocSection(title="Security Architecture", content=self._generate_security_arch_doc()),
                ],
            )
        ]

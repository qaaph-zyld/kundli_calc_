...
class Worker:
    ...
    async def process_task(self, task_id: str, task_func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            result = await loop.run_in_executor(self.executor, lambda: task_func(*args, **kwargs))
            ...
            with self.lock:
                ...
                if self.metrics.completed_tasks == 1:
                    ...
                    self.metrics.avg_response_time = (
                        self.metrics.avg_response_time * (self.metrics.completed_tasks - 1) + execution_time
            ...
            self.logger.error(f"Task {task_id} failed on worker {self.worker_id}: {str(e)}")
            raise
    ...
    def get_load_factor(self) -> float:
        ...
        with self.lock:
            ...
            # Weighted average
            return 0.4 * active_factor + 0.3 * cpu_factor + 0.3 * memory_factor

    def is_available(self) -> bool:
        """Check if worker can accept new tasks"""
        with self.lock:
            return (
                self.metrics.active_tasks < self.max_concurrent_tasks
                and self.metrics.cpu_usage < 80
                and self.metrics.memory_usage < 800  # 800MB limit
            )
...
class LoadBalancer:
    ...
    def __init__(
        ...
        for i in range(max_workers):
            worker = Worker(f"worker_{i}", max_concurrent_tasks=tasks_per_worker)
    ...
    def _select_round_robin(self) -> Optional[Worker]:
        ...
        while True:
            ...
            self.current_worker_idx = (self.current_worker_idx + 1) % self.max_workers
    ...
    def _select_least_connections(self) -> Optional[Worker]:
        ...
        return min(available_workers, key=lambda w: w.metrics.active_tasks)
    ...
    def _select_resource_aware(self) -> Optional[Worker]:
        ...
        return min(available_workers, key=lambda w: w.get_load_factor())
    ...
    def _select_adaptive(self) -> Optional[Worker]:
        ...
        scored_workers = [(w, self._calculate_worker_score(w)) for w in available_workers]
    ...
    def _calculate_worker_score(self, worker: Worker) -> float:
        ...
        reliability_score = metrics["completed_tasks"] / total_tasks if total_tasks > 0 else 1.0

        # Weighted score
        return 0.4 * perf_score + 0.4 * resource_score + 0.2 * reliability_score
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_workers": sum(1 for w in self.workers.values() if w.metrics.active_tasks > 0),
        }

...
class GemstoneSystem:
    ...
    def recommend_gemstone(
        ...
        should_recommend = self._should_recommend_gem(planet, planet_strength, is_benefic, mahadasha_planet)
    ...
    def _should_recommend_gem(self, planet: str, strength: float, is_benefic: bool, mahadasha: Optional[str]) -> bool:
    ...
    def _get_contraindications(self, planet: str, is_benefic: bool) -> List[str]:
        ...
        if not is_benefic:
            contraindications.append(f"{planet} is functional malefic - consult qualified astrologer before wearing")

        if planet == "Saturn":
            contraindications.append("Blue Sapphire can have strong immediate effects - trial period mandatory")

        if planet in ["Rahu", "Ketu"]:
            contraindications.append("Shadow planet gemstone - requires careful consideration")
...
Note: Repeat purification every 6 months for maintaining energy."""
    ...
    def _get_wearing_procedure(
        self, gem: str, metal: str, finger: str, day: str, hora: str, mantra: str, count: int
    ) -> str:
...
**Important:** Gemstone works through cosmic ray transmission. Quality and purity are crucial."""
    ...
    def calculate_suitable_time(
        self, planet: str, location_lat: float, location_lon: float, start_date: datetime
    ) -> Dict[str, Any]:
...
def recommend_gemstones_for_chart(
    ...
    for planet, strength in planet_strengths.items():
        ...
        rec = system.recommend_gemstone(
            planet=planet, planet_strength=strength, is_benefic=is_benefic, mahadasha_planet=current_mahadasha
        )

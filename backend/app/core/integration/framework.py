...
from ..mathematics.framework import EclipticCoordinate, PlanetaryMath, SphericalCoordinate
...
class AstrologicalIntegrator:
    ...
    def __init__(self, mode: IntegrationMode = IntegrationMode.HYBRID, ephemeris_path: Optional[str] = None):
        ...
        if self.mode in [IntegrationMode.SWISS_EPHEMERIS, IntegrationMode.HYBRID]:
            import os

            ephe_path = ephemeris_path if ephemeris_path else os.getenv("EPHE_PATH")
            if ephe_path:
    ...
    def calculate_chart(
        ...
        positions = self._calculate_positions(timestamp, location, chart_type, reference_chart)

        # Calculate house cusps
        houses = self._calculate_houses(timestamp, location)
        ...
        # Calculate dashas
        dashas = self.algorithms.calculate_dasha_periods(positions, timestamp)

        # Calculate strengths
        strengths = self.algorithms.calculate_planetary_strength(positions)

        # Generate interpretation
        interpretation = self.interpreter.interpret_chart(positions, timestamp, location)
    ...
    def calculate_transit_chart(
        self, birth_chart: ChartData, transit_time: datetime, location: Optional[GeoLocation] = None
    ) -> ChartData:
        """Calculate transit chart"""
        return self.calculate_chart(ChartType.TRANSIT, transit_time, location or birth_chart.location, birth_chart)

    def calculate_progression_chart(self, birth_chart: ChartData, progression_date: datetime) -> ChartData:
        """Calculate progression chart"""
        return self.calculate_chart(ChartType.PROGRESSION, progression_date, birth_chart.location, birth_chart)

    def calculate_solar_return(
        self, birth_chart: ChartData, year: int, location: Optional[GeoLocation] = None
    ) -> ChartData:
        ...
        exact_time = self._find_solar_return_time(start_time, target_lon, time_window)

        return self.calculate_chart(ChartType.SOLAR_RETURN, exact_time, location or birth_chart.location, birth_chart)

    def calculate_composite_chart(self, chart1: ChartData, chart2: ChartData) -> ChartData:
        """Calculate composite chart"""

        # Calculate midpoint of times
        mid_timestamp = chart1.timestamp + (chart2.timestamp - chart1.timestamp) / 2

        # Calculate midpoint of locations
        mid_lat = (chart1.location.latitude + chart2.location.latitude) / 2
        mid_lon = (chart1.location.longitude + chart2.location.longitude) / 2
        mid_location = GeoLocation(
            latitude=mid_lat,
            longitude=mid_lon,
            altitude=(
                (chart1.location.altitude + chart2.location.altitude) / 2
                if chart1.location.altitude and chart2.location.altitude
                else None
            ),
        )
        ...
        for body in CelestialBody:
            if body in chart1.positions and body in chart2.positions:
                pos1 = chart1.positions[body]
                pos2 = chart2.positions[body]

                # Calculate midpoint longitude
                mid_lon = self.math.calculate_midpoint(pos1.longitude, pos2.longitude)
        ...
        return self.calculate_chart(
            ChartType.COMPOSITE, mid_timestamp, mid_location, None  # No reference chart for composite
        )

    def _calculate_positions(
        self, timestamp: datetime, location: GeoLocation, chart_type: ChartType, reference_chart: Optional[ChartData]
        ...
        if self.mode == IntegrationMode.SWISS_EPHEMERIS:
            ...
            for body in CelestialBody:
                ...
                result = swe.calc_ut(julian_day, self._to_swe_body(body), flags)
                ...
                positions[body] = PlanetaryPosition(
                    body=body, longitude=lon, latitude=lat, distance=dist, speed=speed, is_retrograde=speed < 0
                )
            ...
            for body in CelestialBody:
                ...
                eq = EclipticCoordinate(
                    longitude=ephem_body.ra * 180 / math.pi, latitude=ephem_body.dec * 180 / math.pi
                )
            ...
            positions = self.calculator.calculate_positions(timestamp, location)
        ...
        if chart_type == ChartType.PROGRESSION:
            ...
            positions = self._progress_positions(
                positions, reference_chart.positions, reference_chart.timestamp, timestamp
            )

        return positions

    def _calculate_houses(self, timestamp: datetime, location: GeoLocation) -> Dict[House, float]:
        """Calculate house cusps"""
        ...
        if self.mode == IntegrationMode.SWISS_EPHEMERIS:
            ...
            cusps, ascmc = swe.houses(julian_day, location.latitude, location.longitude, b"P")  # Placidus
            ...
            # Calculate Ascendant
            houses[House.FIRST] = math.degrees(float(observer.sidereal_time()))

            # Calculate other houses (simplified)
            for i, house in enumerate(House):
                if house != House.FIRST:
                    houses[house] = (houses[House.FIRST] + i * 30) % 360

        else:  # HYBRID or STANDALONE
            # Use built-in calculations
            houses = self.calculator.calculate_houses(timestamp, location)

        return houses

    def _calculate_aspects(self, positions: Dict[CelestialBody, PlanetaryPosition]) -> List[AspectPosition]:
        ...
        for i in range(len(bodies)):
            for j in range(i + 1, len(bodies)):
                ...
                # Check for aspects
                aspect = self.algorithms._check_mutual_aspect(pos1, pos2)
                ...
                if aspect:
                    aspects.append(
                        AspectPosition(
                            ...
                            body2=body2,
                            aspect=aspect,
                            orb=self.math.angular_distance(pos1.longitude, pos2.longitude),
                        )
                    )
    ...
    def _progress_positions(
        ...
        for body, birth_pos in birth_positions.items():
            ...
            progressed_lon = self.math.calculate_progression(birth_pos, progression_time, birth_time)
    ...
    def _find_solar_return_time(
        self, start_time: datetime, target_longitude: float, time_window: timedelta, precision: float = 0.0001
    ) -> datetime:
        """Find exact solar return time"""
        ...
        while (right - left) > timedelta(minutes=1):
            ...
            positions = self._calculate_positions(
                mid, GeoLocation(0, 0), ChartType.TRANSIT, None  # Location doesn't matter for Sun
            )

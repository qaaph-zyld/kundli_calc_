...
class CalculationIntegrationLayer:
    ...
    async def process_calculation_request(self, calculation_id: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.recovery_manager.start_calculation(calculation_id, input_data)

            # Validate input data
            validation_result = await self.validator.validate_house_analysis([input_data])
            if validation_result.validation_status != "PASSED":
                raise ValueError(f"Input validation failed: {validation_result.error_details}")

            # Process calculation with error recovery
            result = await self._process_calculation(calculation_id, input_data)
    ...
    async def _process_calculation(self, calculation_id: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.recovery_manager.update_calculation_state(
                calculation_id, "processing", {"parameters_extracted": True}, "parameter_extraction"
            )

            # Perform house analysis
            result = await self.analysis_engine.analyze_house(
                house=house, occupants=occupants, aspects=aspects, lord=lord, time_of_day=time_of_day
            )

            # Update calculation state
            await self.recovery_manager.update_calculation_state(
                calculation_id, "analyzing", {"analysis_complete": True}, "house_analysis"
            )
            ...
            if validation_result.validation_status != "PASSED":
                raise ValueError(f"Result validation failed: {validation_result.error_details}")

            # Complete calculation
            await self.recovery_manager.complete_calculation(calculation_id, {"result": result.__dict__})
    ...
    def _update_performance_metrics(self, start_time: datetime) -> None:
        ...
        self.metrics.avg_response_time = (total_time + execution_time) / self.metrics.success_count
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.metrics.success_count / self.metrics.request_count if self.metrics.request_count > 0 else 0
            ),
            "error_rate": (
                self.metrics.error_count / self.metrics.request_count if self.metrics.request_count > 0 else 0
            ),

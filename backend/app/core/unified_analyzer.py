...
from .calculations.planetary_strength import PlanetaryStrength, PlanetaryStrengthCalculator
...
class UnifiedAnalyzer:
    ...
    def analyze_chart(
        self, birth_time: datetime, latitude: float, longitude: float, planet_positions: Dict[str, Dict[str, float]]
    ) -> ChartAnalysis:
        """
        ...
        divisional_charts = {d: self.divisional_calc.calculate_chart(birth_time, d, location) for d in divisions}
        ...
        for house in range(1, 13):
            occupants = [{"name": p, **d} for p, d in planet_positions.items() if int(d["longitude"] / 30) + 1 == house]
            ...
            analysis = self.house_analyzer.analyze_house(house=house, occupants=occupants, aspects=aspects, lord=lord)
        ...
        chart_strength = self._calculate_chart_strength(planetary_strengths, house_strengths, aspects, yogas)

        # 8. Generate primary influences and recommendations
        influences = self._determine_primary_influences(planetary_strengths, house_strengths, yogas)
        recommendations = self._generate_recommendations(planetary_strengths, house_strengths, yogas)
    ...
    def _get_house_positions(self, planet_positions: Dict[str, Dict[str, float]]) -> Dict[int, List[str]]:
    ...
    def _get_aspects_to_house(self, house: int, planet_positions: Dict[str, Dict[str, float]]) -> List[Dict[str, Any]]:
        ...
        for planet, data in planet_positions.items():
            ...
            if planet_house != house:  # Don't consider self-aspect
                aspect = {
                    ...
                    "type": self._determine_aspect_type(data["longitude"], house_longitude),
                    "is_applying": self._is_aspect_applying(data["longitude"], house_longitude),
                }
                aspects.append(aspect)

        return aspects

    def _get_house_lord(self, house: int, planet_positions: Dict[str, Dict[str, float]]) -> Dict[str, Any]:
    ...
    def _determine_aspect_type(self, longitude1: float, longitude2: float) -> str:
    ...
    def _is_aspect_applying(self, longitude1: float, longitude2: float) -> bool:
    ...
    def _calculate_chart_strength(
        ...
        weights = {"planetary": 0.35, "house": 0.25, "aspect": 0.20, "yoga": 0.20}
    ...
    def _determine_primary_influences(
        ...
        strong_planets = sorted(planetary_strengths.items(), key=lambda x: x[1].total_strength, reverse=True)[:3]
        influences.extend(f"Strong {p[0]}" for p in strong_planets)

        # Add strongest houses
        strong_houses = sorted(house_strengths.items(), key=lambda x: x[1].total_strength, reverse=True)[:3]
        influences.extend(f"Prominent {h[0]}th house" for h in strong_houses)

        # Add significant yogas
        significant_yogas = sorted(yogas, key=lambda x: x.strength, reverse=True)[:3]
    ...
    def _generate_recommendations(
        ...
        weak_planets = [p for p, s in planetary_strengths.items() if s.total_strength < 50]
        if weak_planets:
            recommendations.append(f"Strengthen {', '.join(weak_planets)} through appropriate remedies")

        # Check for weak houses
        weak_houses = [h for h, s in house_strengths.items() if s.total_strength < 50]
        if weak_houses:
            recommendations.append(f"Focus on areas related to houses {', '.join(map(str, weak_houses))}")

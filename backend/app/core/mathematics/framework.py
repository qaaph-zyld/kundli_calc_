...
class SphericalCoordinate:
    ...
    @classmethod
    def from_cartesian(cls, x: float, y: float, z: float) -> "SphericalCoordinate":
...
class EclipticCoordinate:
    ...
    def to_equatorial(self, obliquity: float) -> "EquatorialCoordinate":
        ...
        return EquatorialCoordinate(right_ascension=α, declination=δ)
...
class EquatorialCoordinate:
    ...
    def to_horizontal(self, location: GeoLocation, sidereal_time: float) -> "HorizontalCoordinate":
        ...
        return HorizontalCoordinate(azimuth=A, altitude=h)
...
class PlanetaryMath:
    ...
    @staticmethod
    def angular_velocity(pos1: PlanetaryPosition, pos2: PlanetaryPosition, time_diff: timedelta) -> float:
        """Calculate angular velocity between two positions"""
        angle_diff = PlanetaryMath.angular_distance(pos1.longitude, pos2.longitude)
    ...
    @staticmethod
    def calculate_phase_angle(body_lon: float, sun_lon: float) -> float:
        """Calculate phase angle relative to Sun"""
        return PlanetaryMath.normalize_angle(body_lon - sun_lon)

    @staticmethod
    def calculate_aspect_orb(pos1: PlanetaryPosition, pos2: PlanetaryPosition, aspect: Aspect) -> float:
        """Calculate aspect orb"""
        actual_angle = PlanetaryMath.angular_distance(pos1.longitude, pos2.longitude)
    ...
    @staticmethod
    def calculate_dignity_score(body: CelestialBody, sign: ZodiacSign, house: House) -> float:
        """Calculate dignity score"""
        from ..astronomical.config import HOUSE_SIGNIFICATIONS, PLANET_PROPERTIES, ZODIAC_PROPERTIES
    ...
    @staticmethod
    def calculate_dispositor_chain(positions: Dict[CelestialBody, PlanetaryPosition]) -> List[List[CelestialBody]]:
    ...
    @staticmethod
    def calculate_progression(birth_pos: PlanetaryPosition, current_date: datetime, birth_date: datetime) -> float:
    ...
    def calculate_solar_arc(
        birth_sun: PlanetaryPosition, current_sun: PlanetaryPosition, birth_pos: PlanetaryPosition
    ) -> float:
        """Calculate solar arc direction"""
        solar_arc = PlanetaryMath.angular_distance(current_sun.longitude, birth_sun.longitude)
        directed_longitude = birth_pos.longitude + solar_arc
        return PlanetaryMath.normalize_angle(directed_longitude)

    @staticmethod
    def calculate_composite(pos1: PlanetaryPosition, pos2: PlanetaryPosition) -> float:
        """Calculate composite position"""
        return PlanetaryMath.calculate_midpoint(pos1.longitude, pos2.longitude)

    @staticmethod
    def calculate_harmonics(
        positions: Dict[CelestialBody, PlanetaryPosition], harmonic: int
    ) -> Dict[CelestialBody, float]:
        """Calculate harmonic positions"""
        harmonic_positions = {}
        for body, pos in positions.items():
            harmonic_pos = PlanetaryMath.calculate_harmonic(pos.longitude, harmonic)

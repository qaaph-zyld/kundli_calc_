...
def test_calculate_houses(calculator):
    ...
    location = Location(latitude=51.4777, longitude=0.0, timezone='Europe/London')  # Greenwich
    
    houses = calculator.calculate_houses(dt, location.latitude, location.longitude)
...
def test_house_systems(calculator):
    ...
    houses_p = calculator.calculate_houses(dt, location.latitude, location.longitude)
    
    # Test Koch
    calculator.house_system = "KOCH"
    houses_k = calculator.calculate_houses(dt, location.latitude, location.longitude)
